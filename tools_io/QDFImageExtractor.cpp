...
int QDFImageExtractor::splitArraySpec(const std::string sArraySpec) {
    ...
    if (iPosLookUp != std::string::npos) {
        if (iPosIndex != std::string::npos) {
            sArrName = sArraySpec.substr(0, iPosIndex - 1);
            ...
            sArrName = sArraySpec.substr(0, iPosLookUp - 1);
            iResult = 0;
        }
...
// extractAll
//
int QDFImageExtractor::extractAll(const std::string sOutPat, const std::string sCompOp, float fTime) {
    int iResult = -1;
...
bool QDFImageExtractor::extractColors(const std::string sColorDesc, uchar *pR, uchar *pG, uchar *pB, uchar *pA) {
    ...
    if ((sColorDesc.length() == 8) && (sColorDesc.front() == '#')) {
