...
void usage(const std::string sApp) {
    ...
    stdprintf("    [-c <operation>] [-r <lon_roll>] [-w <ww>x<hw>+<ox>+<oy> ][-t <timestamp>] [-v]\n");
    stdprintf("or");
    stdprintf("  %s -g <qdf_grid> -f <batch_file>\n", sApp);
    stdprintf("     -s <w>x<h>  -a <arrayspec>,[<arrayspec>]*\n");
    stdprintf("    [-c <operation>] [-r <lon_roll>] [-t <timestamp>] [-v]\n");
    stdprintf("where\n");
    ...
    stdprintf("  timestamp      timestamp to be rendered on image\n");
    stdprintf("  -v             verbose\n");
...
// doSingleImage
//
int doSingleImage(const std::string sQDFData, SurfaceGrid *pSG, const std::string sQDFGrid, const std::string sArrayData, img_prop &ip, const std::string sOutPat, const std::string sCompOp, float fTime, bool bVerbose) {
    int iResult = 0;
    char *pCopy = NULL;
    stringvec vQDFs;
    ...
    if (iResult == 0) {
        ...
        if (pQIE != NULL) {
            pQIE->extractAll(sOutPat, sCompOp, fTime/1000);
            delete pQIE;
        } else {
                    ...
                    if (sDescFile != NULL) {
                        ...
                        if (sOutPat != NULL) {
                            if (sQDFData != NULL) {
                                ...
                                tsl.push_back(desc_entry(iTime, sQDFGrid, sOutPat));
                            }
