...
EQTriangle *EQTriangle::copy() {
    ...
    memcpy((void *)(pEQ->m_pNodes), m_pNodes, m_iNumNodes*sizeof(node));
...
int EQTriangle::createSubdividedTriangle(double dSide, int iSubDiv) {
    ...
    memset((void*)m_pNodes, 0, m_iNumNodes*sizeof(node));
...
//
void EQTriangle::applyQuat(const Quat &qQ) {

    for (gridtype i = 0; i < m_iNumNodes; i++) {
        m_pNodes[i].v =  (!qQ) ^ m_pNodes[i].v;
    } 
...
//
void EQTriangle::shift(const Vec3D &vShift) {
