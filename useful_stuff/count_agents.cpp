...
int main(int iArgC, char *apArgV[]) {
    ...
    if (iArgC > 3) {
        ...
        AgentCounter *pAC = AgentCounter::createInstance(pPop, pGeo, pVeg, pRT);
        if (pAC != NULL) {
            if (pRT == NULL) {
                iResult = pAC->getCellAgentCounts();
                if (iResult == 0) {
                    pAC->writeValidCountsNPP(pOut, NULL);
                }
            } else {
                iResult = pAC->getCellAgentCountsRegion();
                if (iResult == 0) {
                    pAC->writeValidCountsNPP(pOut, NULL);
                }
            }
