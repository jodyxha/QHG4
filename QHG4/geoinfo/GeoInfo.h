...
public:
    ProjType();
    ProjType(int iProjType, double dLambda0, double dPhi0, std::vector<double> vdAdd);
    void copy(const ProjType *pt);
    static ProjType *createPT(const std::string sPDData, bool bDegrees);

    int     m_iProjType; // -> make sure JavaProg has same indexes
    double  m_dLambda0;
    double  m_dPhi0;
    std::vector<double> m_vdAdd;
    ...
    std::string toString(bool bDegrees) const;
    int fromString(const std::string sLine, bool bDegrees);
...
public:
    ...
    static ProjGrid *createPG(const std::string sPGData);
    ...
    unsigned char *deserialize(unsigned char *p);
    char *toString() const;
    int fromString(const std::string sLine);
