...
// writeToHDF
//
int GeoWriter::writeToQDF(const std::string sFileName, int iStep, float fStartTime, const std::string sInfoString) {
    int iResult = -1;
...
int GeoWriter::write(hid_t hFile) {
    ...
    if (hGeoGroup > 0) {
        ...
        if ((iResult == 0) && (m_pGG->m_adAngles != NULL)) {
            iResult = qdf_writeArray(hGeoGroup, GEO_DS_ANGLES,    m_pGG->m_iNumCells*m_pGG->m_iMaxNeighbors, m_pGG->m_adAngles);
        }        
