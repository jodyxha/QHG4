...
// create
//
int Lattice::create(int iNumLinks, const std::string sProjType, const std::string sProjGrid) {
    int iResult = -1;
    if (m_bDeleteGP && (m_pGP != NULL)) {
        delete m_pGP;
    }
    if ((!sProjType.empty()) &&  (!sProjGrid.empty())) {
        ProjType *pPT = new ProjType();
        ProjGrid *pPG = new ProjGrid();
        iResult = pPT->fromString(sProjType, true);
        if (iResult == 0) {
            iResult = pPG->fromString(sProjGrid);
            if (iResult == 0) {
...
//   from Surface
//
int Lattice::save(const std::string sFile) { 
    int iResult = -1;
    if ((m_pGP != NULL) && (m_iNumLinks > 0)) {
        FILE *fOut = fopen(sFile.c_str(), "wt");
        if (fOut != NULL) {
            const ProjGrid  *pPG = m_pGP->getProjGrid();
            const Projector *pPR = m_pGP->getProjector();
	    std::vector<double> vdEmpty;
            ProjType PT(pPR->getID(), pPR->getLambda0(), pPR->getPhi0(), vdEmpty);
            fprintf(fOut, "LTC3\n");
            fprintf(fOut, "NUMLINKS %d\n", m_iNumLinks);
            fprintf(fOut, "PROJT %s\n", PT.toString(true).c_str());
...
//   from Surface
//
int Lattice::load(const std::string sFile) { 
    int iResult = -1;

    LineReader *pLR = LineReader_std::createInstance(sFile, "rt");
    if (pLR != NULL) {
        ProjType *pPT = new ProjType();
        ProjGrid *pPG = new ProjGrid();
        int iNumLinks = -1;
