...
//--------------------------------------------------------------
// findSector
//  an icosahedral face is considered to be a equilateral triangle
//  {A=(0.5,S3/6), B=(-0.5,S3/6), C=(0,-S3/3)}
//  whose center of gravity is at (0,0).
//  The plane is divided into sectors formed by the triangle's
//  three medians (Seitenhalbierende).
//  sector 0 (D) : triangle containing 0 and C left of the Y-axis
//  sector 1 (-) : this should not be happening
//  sector 2 (C) : triangle containing 0, B and P with P.x < 0
//  sector 3 (B) : triangle containing 0, B and P with P.x = 0
//  sector 4 (E) : triangle containing 0 and C right of the Y-axis
//  sector 5 (F) : triangle containing 0, A and P with P.x > 0
//  sector 6 (-) : this should not be happening
//  sector 7 (A) : triangle containing 0, C and P with P.x = 0 (the base tegmark triangle)
// 
int findSector(Vec3D *pV) {
    int iSector = 0;

    // point to the right of the median passing through lower corner?
    if (pV->m_fX >= 0) {
        iSector++;
    }
    iSector <<= 1;
    ...
    // point above the median passing through upper right corner?
    if (pV->m_fY >= pV->m_fX/S3) {
        iSector++;
    }
    iSector <<= 1;

    // point above the median passing through upper left corner?
    if (pV->m_fY >= -pV->m_fX/S3) {
        iSector++;
    }
    return iSector;
