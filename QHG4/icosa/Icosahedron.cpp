...
#pragma omp parallel default(none)              \
    ...
    while (iLevel < iMaxLevel1) {
        ...
        bool bOK = true;
#pragma omp parallel default(none)              \
    shared(chunk, dDLat, dDLon, apFaces, aiCaps, aiFC, apOldFaces, iMax, k, iFacesLeft, bOK)
        {
#pragma omp for schedule(static, chunk)
    ...
    stdprintf("*** switching to phase 2 ***\n");
      ...
      while (iLevel < iMaxLevel) {
        ...
        bool bOK = true;
#pragma omp parallel default(none)              \
    shared(chunk, apFaces, aiCaps, aiFC, apOldFaces, k, iFacesLeft, bOK)
        {
#pragma omp for schedule(static, chunk)
            for (int i = 0; i < k; i++) {
                if (bOK) {
...
//
int Icosahedron::save(const std::string sFile) {
    int iResult = -1;
    BufWriter *pBW =  BufWriter::createInstance(sFile.c_str(), ICO_BUF);
    if (pBW != NULL) {
        // header object
...
//    DIV     ::= "(" <STRUCT><STRUCT><STRUCT><STRUCT> ")"
//
int Icosahedron::load(const std::string sFile) {
    int iResult = 0;
    double dTot1=0;
