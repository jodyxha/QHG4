...
//   Assumption: block has been compacted
template<class T>
int LayerArrBuf<T>::copyBlock(uint iDest, LayerArrBuf<T> *pBuf, uint iOrig, uint iSize) {
    int iResult = LayerBuf<T>::copyBlock(iDest*m_iArraySize, pBuf, iOrig, iSize*m_iArraySize);
    return iResult;
}
