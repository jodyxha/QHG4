...
// getCurMatches
//
stringvec *FileList::getCurMatches(void) {
    stringvec *pv = NULL;
    if (m_iCurIndex == 0) {
...
//   
int FileList::replaceRefs(VEC_INTS &vOrder, stringvec &vs, const char *pOut, char *pOutR) {
    int iResult = 0;
...
//   
int FileList::replaceRefsSimple(stringvec &vs, const char *pOut, char *pOutR) {
    const char *p = pOut;
    char *q = pOutR;
