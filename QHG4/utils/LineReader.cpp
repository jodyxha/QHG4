...
// createInstance
//
LineReader *LineReader_std::createInstance(const std::string sName, const std::string sMode, int iLineLen) {
    LineReader *pLR = new LineReader_std(sName, sMode, iLineLen);
    if (!pLR->isReady()) {
        delete pLR;
        pLR = NULL;
    }
...
// constructor
//
LineReader_std::LineReader_std(const std::string sName, const std::string sMode, int iLineLen)
    : LineReader(iLineLen),
      m_fIn(NULL) {
    
    m_fIn = fopen(sName.c_str(), sMode.c_str());
    if (m_fIn == NULL) {
        xha_fprintf(stderr, "(%d)[LineReader_std::LineReader_std] Error opening file [%s] (%d):%s\n", getpid(), sName, errno, strerror(errno));
    }
    m_bClose = true;
...
LineReader_std::~LineReader_std() {
    if (m_bClose && (m_fIn != NULL)) {
        ...
        if (iResult != 0) {
            xha_fprintf(stderr, "(%d)[LineReader_std::~LineReader_std] Error closing file (%d):%s\n", getpid(), errno, strerror(errno));
        }
    }
}
...
// createInstance
//
LineReader *LineReader_gz::createInstance(const std::string sName, const std::string sMode, int iLineLen) {
    LineReader *pLR = new LineReader_gz(sName, sMode, iLineLen);
    if (!pLR->isReady()) {
        delete pLR;
        pLR = NULL;
    }
...
// constructor
//
LineReader_gz::LineReader_gz(const std::string sName, const std::string sMode, int iLineLen)
    : LineReader(iLineLen),
      m_fIn(NULL) {

    m_bClose = true;
    m_fIn = gzopen(sName.c_str(), sMode.c_str());
}
...
LineReader_gz::~LineReader_gz() {
    if (m_bClose && (m_fIn != NULL)) {
        ...
        if (iResult != Z_OK) {
            xha_fprintf(stderr, "(%d)[LineReader_gz::~LineReader_gz] Error closing files (%d)\n", getpid(), iResult);
        }
    }
}
