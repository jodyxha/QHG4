...
public:
   ...
   ~ParamReader();
    ...
    void display(const std::string sOmit="");
    void display(FILE *fOut, bool bLines, const std::string sOmit="");

    std::string getErrorMessage(int iResult);
    std::string getBadArg() { return m_sBadArg;};
    std::string getBadVal() { return m_sBadVal;};
protected:
    int countWords(const std::string sConfigFile);
    OptionList *m_pOptionList;    
    bool m_bVerbose;
    std::string m_sBadArg;
    std::string m_sBadVal;
    stringvec m_vFreeParams;
    stringvec m_vMissingManadatory;
