...
//
bool Pattern::subMatch(unsigned int iPat, unsigned int iPart,const char *pString, stringvec &vMatches) {
    bool bOK=false;
    const char *p;
    ...
    if (iPart < m_vvPatternParts[iPat].size()) {
        ...
        case '*':
            ...
            if (p != NULL) {
              ...
              bOK = false;
              while ((!bOK) && (p != NULL)) {
                  bOK = subMatch(iPat, iPart+1, p + m_vvPatternParts[iPat][iPart].length(), vMatches);
...
// matchPattern
//
bool Pattern::matchPattern(int iPat, const char *pString, stringvec &vMatches) {
