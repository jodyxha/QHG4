...
OoANavGen2bitPop::OoANavGen2bitPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    ...
    m_pNPPCap = new NPPCapacity<OoANavGen2bitAgent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride); 

    // prepare parameters for MultiEvaluator, which will compute actual carrying capacity
    MultiEvaluator<OoANavGen2bitAgent>::evaluatorinfos mEvalInfo;

    // add altitude evaluation - output is NULL because it will be set by MultiEvaluator
    SingleEvaluator<OoANavGen2bitAgent> *pSEAlt = new SingleEvaluator<OoANavGen2bitAgent>(this, m_pCG, "Alt", NULL, (double*)m_pCG->m_pGeography->m_adAltitude, "AltCapPref", true, EVENT_ID_GEO);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<OoANavGen2bitAgent>*>("Multi_weight_alt", pSEAlt));

    // add NPP evaluation - output is NULL because it will be set by MultiEvaluator
    SingleEvaluator<OoANavGen2bitAgent> *pSECap = new SingleEvaluator<OoANavGen2bitAgent>(this, m_pCG, "NPP", NULL, m_adCapacities, "NPPPref", true, EVENT_ID_VEG);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<OoANavGen2bitAgent>*>("Multi_weight_npp", pSECap));

    m_pME = new MultiEvaluator<OoANavGen2bitAgent>(this, m_pCG, "Alt+NPP", m_adEnvWeights, mEvalInfo, MODE_ADD_SIMPLE, true); // true: delete evaluators


    m_pVerVarK = new VerhulstVarK<OoANavGen2bitAgent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride);
    ...
    //    m_pCM = new ConfinedMove<OoANavGen2bitAgent>(this, m_pCG, "");
...
// setParams
//
int OoANavGen2bitPop::setParams(const std::string sParams) {
    int iResult = 0;
...
int OoANavGen2bitPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    ...
    OoANavGen2bitAgent &newAgent = m_aAgents[iAgent];
    newAgent.m_fAge = 0.0;
...
//
int OoANavGen2bitPop::copyAdditionalDataQDF(int iStart, int iCount, SPopulation *pPop) {
    OoANavGen2bitPop *pO = static_cast<OoANavGen2bitPop *>(pPop);
