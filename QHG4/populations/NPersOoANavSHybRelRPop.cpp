...
#include "NPersOoANavSHybRelRPop.h"
...
// constructor
//
NPersOoANavSHybRelRPop::NPersOoANavSHybRelRPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    : SPopulation<NPersOoANavSHybRelRAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState, aiSeeds),
    ...
    m_pLE = new LocEnv<NPersOoANavSHybRelRAgent>(this, m_pCG, "", m_apWELL, m_pAgentController); 
    m_pLE->init();
    // addObserver(m_pLE);
    m_pPPM = new PrivParamMix<NPersOoANavSHybRelRAgent>(this, m_pCG, "", m_apWELL); 
   
    m_pHybBirthDeathRel = new NPersHybBirthDeathRel<NPersOoANavSHybRelRAgent>(this, m_pCG, "", m_apWELL, m_aiNumBirths);


    m_pWM = new NPersRandomMove<NPersOoANavSHybRelRAgent>(this, m_pCG, "", m_apWELL/*, m_pLE*/);

    m_pPair = new RandomPair<NPersOoANavSHybRelRAgent>(this, m_pCG, "", m_apWELL);

    m_pGO = new GetOld<NPersOoANavSHybRelRAgent>(this, m_pCG, "");

    m_pOAD = new PersOldAgeDeath<NPersOoANavSHybRelRAgent>(this, m_pCG, "", m_apWELL);

    m_pFert = new PersFertility<NPersOoANavSHybRelRAgent>(this, m_pCG, "");

    m_pNavigate = new Navigate<NPersOoANavSHybRelRAgent>(this, m_pCG, "", m_apWELL);
...
// destructor
//
NPersOoANavSHybRelRPop::~NPersOoANavSHybRelRPop() {
...
// setParams
//
int NPersOoANavSHybRelRPop::setParams(const std::string sParams) {
    int iResult = 0;
...
// preLoop
//
int NPersOoANavSHybRelRPop::preLoop() {
    // here we should createGenomes (if this is a run started from scratch)
    int iResult = SPopulation<NPersOoANavSHybRelRAgent>::preLoop();
...
//  initializeStep
//
int NPersOoANavSHybRelRPop::initializeStep(float fCurTime) {
...
//  finalizeStep
//
int NPersOoANavSHybRelRPop::finalizeStep() {
...
//
int NPersOoANavSHybRelRPop::updateEvent(int iEventID, char *pData, float fT) {
...
// flushEvents
//
void NPersOoANavSHybRelRPop::flushEvents(float fT) {
    if (m_bPendingEvents) {
...
//
int NPersOoANavSHybRelRPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    int iResult = 0;    
    NPersOoANavSHybRelRAgent &curMother = m_aAgents[iMother];
    NPersOoANavSHybRelRAgent &curFather = m_aAgents[iFather];
    
    NPersOoANavSHybRelRAgent &curAgent = m_aAgents[iAgent];
    curAgent.m_fAge = 0.0;
...
//
int NPersOoANavSHybRelRPop::addPopSpecificAgentData(int iAgentIndex, char **ppData) {
    int iResult = 0;
    NPersOoANavSHybRelRAgent &newAgent  = m_aAgents[iAgentIndex];
...
//
void NPersOoANavSHybRelRPop::addPopSpecificAgentDataTypeQDF(hid_t *hAgentDataType) {

    NPersOoANavSHybRelRAgent agent;
