...
#include "OoANavSHybSelYchMTDPop.h"
...
// constructor
//
OoANavSHybSelYchMTDPop::OoANavSHybSelYchMTDPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    : SPopulation<OoANavSHybSelYchMTDAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState, aiSeeds),
    ...
    m_pLE = new LocEnv<OoANavSHybSelYchMTDAgent>(this, m_pCG, "", m_apWELL, m_pAgentController); 
    m_pLE->init();
    // addObserver(m_pLE);
    m_pPPM = new PrivParamMix<OoANavSHybSelYchMTDAgent>(this, m_pCG, "", m_apWELL); 
   
    m_pHybBirthDeathRel = new NPersZHybBirthDeathRel<OoANavSHybSelYchMTDAgent>(this, m_pCG, "", m_apWELL, m_aiNumBirths);


    m_pWM = new NPersWeightedMove<OoANavSHybSelYchMTDAgent>(this, m_pCG, "", m_apWELL, m_pLE);

    m_pPair = new HybSelPair<OoANavSHybSelYchMTDAgent>(this, m_pCG, "");

    m_pGO = new GetOld<OoANavSHybSelYchMTDAgent>(this, m_pCG, "");

    m_pOAD = new PersOldAgeDeath<OoANavSHybSelYchMTDAgent>(this, m_pCG, "", m_apWELL);

    m_pFert = new PersFertility<OoANavSHybSelYchMTDAgent>(this, m_pCG, "");

    m_pNavigate = new Navigate<OoANavSHybSelYchMTDAgent>(this, m_pCG, "", m_apWELL);
...
// destructor
//
OoANavSHybSelYchMTDPop::~OoANavSHybSelYchMTDPop() {
...
//
int OoANavSHybSelYchMTDPop::setParams(const std::string sParams) {
    int iResult = 0;
    if (!sParams.empty()) {
        stringvec vParts;
        ...
        if (iNum == 2) {
                    ...
                    if (iNumSub1 == 2) {
                        ...
                        if (strToNum(vSubs1[1], &dSDev)) {
                            ...
                            if (sAvgName == "MaternalContribAvg") {
                                ...
                                xha_printf("[OoANavSHybSelYchMTDPop::setParams] unknown avg contribution [%s]\n", sAvgName);
                                iResult = -1;
                            }
                            ...
                            if (sDevName == "MaternalContribSDev") {
                                ...
                                xha_printf("[OoANavSHybSelYchMTDPop::setParams] unknown sdev contribution [%s]\n", sDevName);
                                iResult = -1;
                            }
  
                        } else {
                            xha_printf("[OoANavSHybSelYchMTDPop::setParams] bad value for pcsdev [%s]\n", vSubs1[1]);
                            iResult = -1;
                        }
                    } else {
                        xha_printf("[OoANavSHybSelYchMTDPop::setParams] expected <ParentContribSDev>=<pcsdev> [%s]\n", sParams);
                        iResult = -1;
                    }
                } else {
                    xha_printf("[OoANavSHybSelYchMTDPop::setParams] bad value for pcavg [%s]\n", vSubs0[1]);
                    iResult = -1;
                }
            ...
            xha_printf("[OoANavSHybSelYchMTDPop::setParams] expected <ParentContribAvg>=<pcavg>+<ParentContribSDev>=<pcsdev> [%s]\n", sParams);
            iResult = -1;

        }
    } else {
        xha_printf("[OoANavSHybSelYchMTDPop::setParams] empty params: do nothing\n");
        iResult = 0;
    }
...
// preLoop
//
int OoANavSHybSelYchMTDPop::preLoop() {
    //    xha_printf("!!!intertest!!! OoANavSHybSelYchMTDPop::preLoop\n");
    // here we should createGenomes (if this is a run started from scratch)
    int iResult = SPopulation<OoANavSHybSelYchMTDAgent>::preLoop();
...
//  initializeStep
//
int OoANavSHybSelYchMTDPop::initializeStep(float fCurTime) {
...
//  finalizeStep
//
int OoANavSHybSelYchMTDPop::finalizeStep() {
...
//
int OoANavSHybSelYchMTDPop::updateEvent(int iEventID, char *pData, float fT) {
...
// flushEvents
//
void OoANavSHybSelYchMTDPop::flushEvents(float fT) {
    if (m_bPendingEvents) {
...
//
int OoANavSHybSelYchMTDPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    int iResult = 0;    
    OoANavSHybSelYchMTDAgent &curMother = m_aAgents[iMother];
    OoANavSHybSelYchMTDAgent &curFather = m_aAgents[iFather];
    
    OoANavSHybSelYchMTDAgent &curAgent = m_aAgents[iAgent];
    curAgent.m_fAge = 0.0;
...
//
int OoANavSHybSelYchMTDPop::addPopSpecificAgentData(int iAgentIndex, char **ppData) {
    int iResult = 0;
    OoANavSHybSelYchMTDAgent &newAgent = m_aAgents[iAgentIndex];
    if (iResult == 0) {
...
//
void OoANavSHybSelYchMTDPop::addPopSpecificAgentDataTypeQDF(hid_t *hAgentDataType) {

    OoANavSHybSelYchMTDAgent agent;
