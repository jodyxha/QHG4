...
FlatExpPop::FlatExpPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    ...
    m_pNPPCap = new NPPCapacity<FlatExpAgent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride); 

    // prepare parameters for MultiEvaluator, which will compute actual carrying capacity
    MultiEvaluator<FlatExpAgent>::evaluatorinfos mEvalInfo;

    // add altitude evaluation - output is NULL because it will be set by MultiEvaluator
    SingleEvaluator<FlatExpAgent> *pSEAlt = new SingleEvaluator<FlatExpAgent>(this, m_pCG, "Alt", NULL, (double*)m_pCG->m_pGeography->m_adAltitude, "AltCapPref", true, EVENT_ID_GEO);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<FlatExpAgent>*>("Multi_weight_alt", pSEAlt));

    // add NPP evaluation - output is NULL because it will be set by MultiEvaluator
    SingleEvaluator<FlatExpAgent> *pSECap = new SingleEvaluator<FlatExpAgent>(this, m_pCG, "NPP", NULL, m_adCapacities, "NPPPref", true, EVENT_ID_VEG);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<FlatExpAgent>*>("Multi_weight_npp", pSECap));

    m_pME = new MultiEvaluator<FlatExpAgent>(this, m_pCG, "Alt+NPP", m_adEnvWeights, mEvalInfo, MODE_ADD_SIMPLE, true);  // true: delete evaluators


    m_pVerhulst = new Verhulst<FlatExpAgent>(this, m_pCG, "", m_apWELL);
    ...
    //    m_pCM = new ConfinedMove<FlatExpAgent>(this, m_pCG, "");
...
// setParams
//
int FlatExpPop::setParams(const std::string sParams) {
    int iResult = 0;
...
int FlatExpPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    ...
    FlatExpAgent &newAgent = m_aAgents[iAgent];
    newAgent.m_fAge = 0.0;
