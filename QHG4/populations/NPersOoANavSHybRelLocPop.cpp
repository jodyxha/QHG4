...
#include "NPersOoANavSHybRelLocPop.h"
...
// constructor
//
NPersOoANavSHybRelLocPop::NPersOoANavSHybRelLocPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    : SPopulation<NPersOoANavSHybRelLocAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState, aiSeeds),
    ...
    m_pLE = new LocEnv<NPersOoANavSHybRelLocAgent>(this, m_pCG, "", m_apWELL, m_pAgentController); 
    m_pLE->init();
    // addObserver(m_pLE);
    m_pPPM = new PrivParamMix<NPersOoANavSHybRelLocAgent>(this, m_pCG, "", m_apWELL); 
   
    m_pHybBirthDeathRel = new NPersHybBirthDeathRel<NPersOoANavSHybRelLocAgent>(this, m_pCG, "", m_apWELL, m_aiNumBirths);


    m_pWM = new NPersWeightedMove<NPersOoANavSHybRelLocAgent>(this, m_pCG, "", m_apWELL, m_pLE);

    m_pPair = new RandomPair<NPersOoANavSHybRelLocAgent>(this, m_pCG, "", m_apWELL);

    m_pGO = new GetOld<NPersOoANavSHybRelLocAgent>(this, m_pCG, "");

    m_pOAD = new PersOldAgeDeath<NPersOoANavSHybRelLocAgent>(this, m_pCG, "", m_apWELL);

    m_pFert = new PersFertility<NPersOoANavSHybRelLocAgent>(this, m_pCG, "");

    m_pNavigate = new Navigate<NPersOoANavSHybRelLocAgent>(this, m_pCG, "", m_apWELL);
...
// destructor
//
NPersOoANavSHybRelLocPop::~NPersOoANavSHybRelLocPop() {
...
// setParams
//
int NPersOoANavSHybRelLocPop::setParams(const std::string sParams) {
    int iResult = 0;
...
// preLoop
//
int NPersOoANavSHybRelLocPop::preLoop() {
    // here we should createGenomes (if this is a run started from scratch)
    int iResult = SPopulation<NPersOoANavSHybRelLocAgent>::preLoop();
...
//  initializeStep
//
int NPersOoANavSHybRelLocPop::initializeStep(float fCurTime) {
...
//  finalizeStep
//
int NPersOoANavSHybRelLocPop::finalizeStep() {
...
//
int NPersOoANavSHybRelLocPop::updateEvent(int iEventID, char *pData, float fT) {
...
// flushEvents
//
void NPersOoANavSHybRelLocPop::flushEvents(float fT) {
    if (m_bPendingEvents) {
...
//
int NPersOoANavSHybRelLocPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    int iResult = 0;    
    NPersOoANavSHybRelLocAgent &curMother = m_aAgents[iMother];
    NPersOoANavSHybRelLocAgent &curFather = m_aAgents[iFather];
    
    NPersOoANavSHybRelLocAgent &curAgent = m_aAgents[iAgent];
    curAgent.m_fAge = 0.0;
    ...
    // the agent has not moved yet, so there
    curAgent.m_iBirthCell = curAgent.m_ulCellID;
...
//
int NPersOoANavSHybRelLocPop::addPopSpecificAgentData(int iAgentIndex, char **ppData) {
    int iResult = 0;
    NPersOoANavSHybRelLocAgent &newAgent  = m_aAgents[iAgentIndex];
    ...
    // we use the agent's location at start as BirthCell: we can use the same dat file 
    newAgent.m_iBirthCell =  newAgent.m_ulCellID;
    // alternatively, put birthcell in dat file just after age, then:
    //   iResult = addAgentDataSingle(ppData, &newAgent.m_iBirthCell);
...
//
void NPersOoANavSHybRelLocPop::addPopSpecificAgentDataTypeQDF(hid_t *hAgentDataType) {

    NPersOoANavSHybRelLocAgent agent;
    H5Tinsert(*hAgentDataType, "Age",           qoffsetof(agent, m_fAge), H5T_NATIVE_FLOAT);
    H5Tinsert(*hAgentDataType, "BirthCell",     qoffsetof(agent, m_iBirthCell), H5T_NATIVE_INT);
