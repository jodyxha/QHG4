...
PDAltPredPop::PDAltPredPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    ...
    MultiEvaluator<PDAltPredAgent>::evaluatorinfos mEvalInfo;
     
    // add altitude evaluation
    ShareEvaluator<PDAltPredAgent> *pNSH0 = new ShareEvaluator<PDAltPredAgent>(this, m_pCG, "Prey0Mass", NULL, true, EVENT_ID_NONE);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<PDAltPredAgent>*>("ShareWeight0", pNSH0));

    // add altitude evaluation
    ShareEvaluator<PDAltPredAgent> *pNSH1 = new ShareEvaluator<PDAltPredAgent>(this, m_pCG, "Prey1Mass", NULL, true, EVENT_ID_NONE);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<PDAltPredAgent>*>("ShareWeight1", pNSH1));

    // add altitude evaluation
    SingleEvaluator<PDAltPredAgent> *pNSEGeo = new SingleEvaluator<PDAltPredAgent>(this, m_pCG, "Alt", NULL, pAlt, "AltPreference", true, EVENT_ID_GEO);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<PDAltPredAgent>*>("Multi_weight_alt", pNSEGeo));

    m_pME   = new MultiEvaluator<PDAltPredAgent>(this, m_pCG, "Alt+Masses", m_adEnvWeights, mEvalInfo, MODE_ADD_SIMPLE, true);  // true: delete evaluators
...
#pragma omp parallel 
    ...
    m_prio.addAction(m_pBB);
    m_prio.addAction(m_pPDHU);
...
int PDAltPredPop::preLoop() {
    ...
    if (iResult == 0) {
        ...
        xha_printf("[PDPredPop::preLoop] Share m_afMassArray[0] (%p) as [%s]\n", m_afMassArray[0], sArrName);
