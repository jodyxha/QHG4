...
OoAConfGen2BPop::OoAConfGen2BPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    ...
    m_pNPPCap = new NPPCapacity<OoAConfGen2BAgent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride); 

    // prepare parameters for MultiEvaluator, which will compute actual carrying capacity
    MultiEvaluator<OoAConfGen2BAgent>::evaluatorinfos mEvalInfo;

    // add altitude evaluation - output is NULL because it will be set by MultiEvaluator
    SingleEvaluator<OoAConfGen2BAgent> *pSEAlt = new SingleEvaluator<OoAConfGen2BAgent>(this, m_pCG, "Alt", NULL, (double*)m_pCG->m_pGeography->m_adAltitude, "AltCapPref", true, EVENT_ID_GEO);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<OoAConfGen2BAgent>*>("Multi_weight_alt", pSEAlt));

    // add NPP evaluation - output is NULL because it will be set by MultiEvaluator
    SingleEvaluator<OoAConfGen2BAgent> *pSECap = new SingleEvaluator<OoAConfGen2BAgent>(this, m_pCG, "NPP", NULL, m_adCapacities, "NPPPref", true, EVENT_ID_VEG);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<OoAConfGen2BAgent>*>("Multi_weight_npp", pSECap));

    m_pME = new MultiEvaluator<OoAConfGen2BAgent>(this, m_pCG, "Alt+NPP", m_adEnvWeights, mEvalInfo, MODE_ADD_SIMPLE, true); // true: delete evaluators


    m_pVerVarK = new VerhulstVarK<OoAConfGen2BAgent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride);
    ...
    m_pCM = new ConfinedMove<OoAConfGen2BAgent>(this, m_pCG, "");
...
// setParams
//
int OoAConfGen2BPop::setParams(const std::string sParams) {
    int iResult = 0;
...
int OoAConfGen2BPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    ...
    OoAConfGen2BAgent &newAgent  = m_aAgents[iAgent];
...
//
int OoAConfGen2BPop::copyAdditionalDataQDF(int iStart, int iCount, SPopulation *pPop) {
    OoAConfGen2BPop *pO = static_cast<OoAConfGen2BPop *>(pPop);
