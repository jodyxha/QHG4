...
OoANavGen2bitRandDistrPop::OoANavGen2bitRandDistrPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    ...
    m_pNPPCap = new NPPCapacity<OoANavGen2bitRandDistrAgent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride); 

    // prepare parameters for MultiEvaluator, which will compute actual carrying capacity
    MultiEvaluator<OoANavGen2bitRandDistrAgent>::evaluatorinfos mEvalInfo;

    // add altitude evaluation - output is NULL because it will be set by MultiEvaluator
    SingleEvaluator<OoANavGen2bitRandDistrAgent> *pSEAlt = new SingleEvaluator<OoANavGen2bitRandDistrAgent>(this, m_pCG, "Alt", NULL, (double*)m_pCG->m_pGeography->m_adAltitude, "AltCapPref", true, EVENT_ID_GEO);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<OoANavGen2bitRandDistrAgent>*>("Multi_weight_alt", pSEAlt));

    // add NPP evaluation - output is NULL because it will be set by MultiEvaluator
    SingleEvaluator<OoANavGen2bitRandDistrAgent> *pSECap = new SingleEvaluator<OoANavGen2bitRandDistrAgent>(this, m_pCG, "NPP", NULL, m_adCapacities, "NPPPref", true, EVENT_ID_VEG);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<OoANavGen2bitRandDistrAgent>*>("Multi_weight_npp", pSECap));

    m_pME = new MultiEvaluator<OoANavGen2bitRandDistrAgent>(this, m_pCG, "Alt+NPP", m_adEnvWeights, mEvalInfo, MODE_ADD_SIMPLE, true);  // true: delete evaluators


    m_pVerVarK = new VerhulstVarK<OoANavGen2bitRandDistrAgent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride);
    ...
    //    m_pCM = new ConfinedMove<OoANavGen2bitRandDistrAgent>(this, m_pCG, "");
...
// setParams
//
int OoANavGen2bitRandDistrPop::setParams(const std::string sParams) {
    int iResult = 0;
...
int OoANavGen2bitRandDistrPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    ...
    OoANavGen2bitRandDistrAgent &newAgent = m_aAgents[iAgent];
    newAgent.m_fAge = 0.0;
...
//
int OoANavGen2bitRandDistrPop::copyAdditionalDataQDF(int iStart, int iCount, SPopulation *pPop) {
    OoANavGen2bitRandDistrPop *pO = static_cast<OoANavGen2bitRandDistrPop *>(pPop);
