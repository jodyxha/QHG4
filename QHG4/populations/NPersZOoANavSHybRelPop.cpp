...
#include "NPersZOoANavSHybRelPop.h"

static bool g_bFirst = true;
static double MATC_AVG  = 0.5;
//static double MATC_SDEV = 0.05;
static double MATC_SDEV = 0.05231997;
static double PATC_AVG  = 0.5;
//static double PATC_SDEV = 0.06;
static double PATC_SDEV = 0.0599249;

//----------------------------------------------------------------------------
// constructor
//
NPersZOoANavSHybRelPop::NPersZOoANavSHybRelPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    : SPopulation<NPersZOoANavSHybRelAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState, aiSeeds),
    ...
    m_pLE = new LocEnv<NPersZOoANavSHybRelAgent>(this, m_pCG, "", m_apWELL, m_pAgentController); 
    m_pLE->init();
    // addObserver(m_pLE);
    m_pPPM = new PrivParamMix<NPersZOoANavSHybRelAgent>(this, m_pCG, "", m_apWELL); 
   
    m_pHybBirthDeathRel = new NPersZHybBirthDeathRel<NPersZOoANavSHybRelAgent>(this, m_pCG, "", m_apWELL, m_aiNumBirths);


    m_pWM = new NPersWeightedMove<NPersZOoANavSHybRelAgent>(this, m_pCG, "", m_apWELL, m_pLE);

    m_pPair = new RandomPair<NPersZOoANavSHybRelAgent>(this, m_pCG, "", m_apWELL);

    m_pGO = new GetOld<NPersZOoANavSHybRelAgent>(this, m_pCG, "");

    m_pOAD = new PersOldAgeDeath<NPersZOoANavSHybRelAgent>(this, m_pCG, "", m_apWELL);

    m_pFert = new PersFertility<NPersZOoANavSHybRelAgent>(this, m_pCG, "");

    m_pNavigate = new Navigate<NPersZOoANavSHybRelAgent>(this, m_pCG, "", m_apWELL);
...
// destructor
//
NPersZOoANavSHybRelPop::~NPersZOoANavSHybRelPop() {
...
//
int NPersZOoANavSHybRelPop::setParams(const std::string sParams) {
    int iResult = 0;
    if (!sParams.empty()) {
        stringvec vParts;
...
// preLoop
//
int NPersZOoANavSHybRelPop::preLoop() {
    // here we should createGenomes (if this is a run started from scratch)
    int iResult = SPopulation<NPersZOoANavSHybRelAgent>::preLoop();
...
//  initializeStep
//
int NPersZOoANavSHybRelPop::initializeStep(float fCurTime) {
...
//  finalizeStep
//
int NPersZOoANavSHybRelPop::finalizeStep() {
...
//
int NPersZOoANavSHybRelPop::updateEvent(int iEventID, char *pData, float fT) {
...
// flushEvents
//
void NPersZOoANavSHybRelPop::flushEvents(float fT) {
    if (m_bPendingEvents) {
...
//
int NPersZOoANavSHybRelPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    int iResult = 0;    
    NPersZOoANavSHybRelAgent &curMother = m_aAgents[iMother];
    NPersZOoANavSHybRelAgent &curFather = m_aAgents[iFather];
    
    NPersZOoANavSHybRelAgent &curAgent = m_aAgents[iAgent];
    curAgent.m_fAge = 0.0;
...
//
int NPersZOoANavSHybRelPop::addPopSpecificAgentData(int iAgentIndex, char **ppData) {
    int iResult = 0;
    NPersZOoANavSHybRelAgent &newAgent  = m_aAgents[iAgentIndex];
...
//
void NPersZOoANavSHybRelPop::addPopSpecificAgentDataTypeQDF(hid_t *hAgentDataType) {

    NPersZOoANavSHybRelAgent agent;
