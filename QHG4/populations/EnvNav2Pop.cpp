...
#include "Navigate2.cpp"

#include "EnvNav2Pop.h"


EnvNav2Pop::EnvNav2Pop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds)
    : SPopulation<EnvNav2Agent>(pCG, pPopFinder, iLayerSize, apIDG, aulState, aiSeeds) {
    ...
    MultiEvaluator<EnvNav2Agent>::evaluatorinfos mEvalInfo;

    // add altitude evaluation - output is NULL because it will be set by MultiEvaluator
    SingleEvaluator<EnvNav2Agent> *pSEAlt = new SingleEvaluator<EnvNav2Agent>(this, m_pCG, "Alt", NULL, (double*)m_pCG->m_pGeography->m_adAltitude, "AltPref", true, EVENT_ID_GEO);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<EnvNav2Agent>*>("Multi_weight_alt", pSEAlt));

    // add NPP evaluation - output is NULL because it will be set by MultiEvaluator
    SingleEvaluator<EnvNav2Agent> *pSECap = new SingleEvaluator<EnvNav2Agent>(this, m_pCG, "NPP", NULL, m_adCapacities, "", true, EVENT_ID_VEG);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<EnvNav2Agent>*>("Multi_weight_npp", pSECap));

    m_pME      = new MultiEvaluator<EnvNav2Agent>(this, m_pCG, "NPP+Alt", m_adEnvWeights, mEvalInfo, MODE_ADD_SIMPLE, true);  // true: delete evaluators
    m_pNPPCap  = new NPPCapacity<EnvNav2Agent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride);

    m_pWM       = new WeightedMove<EnvNav2Agent>(this, m_pCG, "", m_apWELL, m_adEnvWeights);

    m_pGO       = new GetOld<EnvNav2Agent>(this, m_pCG, "");
    m_pOAD      = new OldAgeDeath<EnvNav2Agent>(this, m_pCG, "", m_apWELL);
    m_pFert     = new Fertility<EnvNav2Agent>(this, m_pCG, "");
    m_pVerVarK  = new VerhulstVarK<EnvNav2Agent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride);
    m_pPair     = new RandomPair<EnvNav2Agent>(this, m_pCG, "", m_apWELL);
...
//
int EnvNav2Pop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    int iResult = 0;
...
//
int EnvNav2Pop::updateEvent(int iEventID, char *pData, float fT) {
...
//
int EnvNav2Pop::addPopSpecificAgentData(int iAgentIndex, char **ppData) {
    int iResult = 0;
    if (iResult == 0) {
...
//
void EnvNav2Pop::addPopSpecificAgentDataTypeQDF(hid_t *hAgentDataType) {
...
//
int EnvNav2Pop::writeAdditionalDataQDF(hid_t hSpeciesGroup) {
    int iResult = 0;
...
// flushEvents
//
void EnvNav2Pop::flushEvents(float fT) {
