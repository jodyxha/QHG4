...
OoASelPop::OoASelPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    ...
    m_pNPPCap = new NPPCapacity<OoASelAgent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride); 
    ...
    SingleEvaluator<OoASelAgent> *pSECap = new SingleEvaluator<OoASelAgent>(this, m_pCG, "NPP", NULL, m_adCapacities, "NPPPref", true, EVENT_ID_VEG);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<OoASelAgent>*>("Multi_weight_npp", pSECap));

    m_pME = new MultiEvaluator<OoASelAgent>(this, m_pCG, "Alt+NPP", m_adEnvWeights, mEvalInfo, MODE_ADD_SIMPLE, true);  // true: delete evaluators


    m_pVerVarK = new VerhulstVarK<OoASelAgent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride);
    ...
    //    m_pCM = new ConfinedMove<OoASelAgent>(this, m_pCG, "");
...
// setParams
//
int OoASelPop::setParams(const std::string sParams) {
    int iResult = 0;
