...
OoANavPop::OoANavPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    ...
    m_pNPPCap = new NPPCapacity<OoANavAgent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride); 

    // prepare parameters for MultiEvaluator, which will compute actual carrying capacity
    MultiEvaluator<OoANavAgent>::evaluatorinfos mEvalInfo;

    // add altitude evaluation - output is NULL because it will be set by MultiEvaluator
    // ori SingleEvaluator<OoANavAgent> *pSEAlt = new SingleEvaluator<OoANavAgent>(this, m_pCG, "Alt", NULL, (double*)m_pCG->m_pGeography->m_adAltitude, "AltCapPref", true, EVENT_ID_GEO);
    SingleEvaluator<OoANavAgent> *pSEAlt = new SingleEvaluator<OoANavAgent>(this, m_pCG, "Alt", NULL, (double*)m_pCG->m_pGeography->m_adAltitude, "AltCapPref", false, EVENT_ID_GEO);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<OoANavAgent>*>("Multi_weight_alt", pSEAlt));

    // add NPP evaluation - output is NULL because it will be set by MultiEvaluator
    // ori SingleEvaluator<OoANavAgent> *pSECap = new SingleEvaluator<OoANavAgent>(this, m_pCG, "NPP", NULL, m_adCapacities, "NPPPref", true, EVENT_ID_VEG);
    SingleEvaluator<OoANavAgent> *pSECap = new SingleEvaluator<OoANavAgent>(this, m_pCG, "NPP", NULL, m_adCapacities, "NPPPref", false, EVENT_ID_VEG);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<OoANavAgent>*>("Multi_weight_npp", pSECap));

    // ori printf("Creating additive MultiEvaluator\n");
    printf("Creating multiplicative MultiEvaluator\n");
    //m_pME = new MultiEvaluator<OoANavAgent>(this, m_pCG, "Alt+NPP", m_adEnvWeights, mEvalInfo, MODE_ADD_SIMPLE, true);  // true: delete evaluators
    m_pME = new MultiEvaluator<OoANavAgent>(this, m_pCG, "Alt+NPP", m_adEnvWeights, mEvalInfo, MODE_MUL_SIMPLE, true);  // true: delete evaluators
    addObserver(m_pME);


    m_pVerVarK = new VerhulstVarK<OoANavAgent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride);
    ...
    //    m_pCM = new ConfinedMove<OoANavAgent>(this, m_pCG, "");
...
// setParams
//
int OoANavPop::setParams(const std::string sParams) {
    int iResult = 0;
...
int OoANavPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    ...
    @@*/
    OoANavAgent &newAgent = m_aAgents[iAgent];
    newAgent.m_fAge = 0.0;
