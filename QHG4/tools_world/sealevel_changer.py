...
def test(args):
    if (len(args) > 4):
        try:
            ...
            t0 = slc.findLastTimeWithLevel(float(args[4]), 120)
...
if __name__ == '__main__':
    ...
    if (len(argv) > 5):
        try:
            ...
            iResult = slc.setSeaLevel(argv[5], time)
        #-- end except
    else:
        print("Usage:")
        print("  %s <sealevelfile> <t_col> <h_col> <time> <qdf_file>" % argv[0])
        print("where")
        print("  sealevelfile    an ascii file containingrelativer sea levels")
        print("  t_col           column of time values")
        print("  h_col           column of selevel values")
        print("  time            time in ky b.p.")
        print("  qdf_file        qdf_file to be modified")
    #-- end if
    exit(iResult)
