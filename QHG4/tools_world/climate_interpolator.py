...
class ClimateInterpolator:
    ...
    #-----------------------------------------------------------------------------
    #-- doInterpolationTemp
    #--   do bivariate interpolations for temp
    #--   onto the grid defined by qLon and qLat
    #--
    def doInterpolationTemp(self, target_time, qLon, qLat):
        temp_tmp = self.doTimeInterpolationSingle(self.interp_temp, target_time)
        if not temp_tmp is None:
            temp_new  = grid_utils.doGridInterpolation(qLon, qLat, self.longs, self.lats, temp_tmp, np.float64, True)
        #-- end if
        return temp_new
    ...
    #-----------------------------------------------------------------------------
    #-- doInterpolationRain
    #--   do bivariate interpolations for rain
    #--   onto the grid defined by qLon and qLat
    #--
    def doInterpolationRain(self, target_time, qLon, qLat):
        rain_tmp = self.doTimeInterpolationSingle(self.interp_rain, target_time)
        if not rain_tmp is None:
            rain_new  = grid_utils.doGridInterpolation(qLon, qLat, self.longs, self.lats, rain_tmp, np.float64, True)
        #-- end if
        return rain_new
    #-- end def
    ...
    #-----------------------------------------------------------------------------
    #-- doTimeInterpolationSingle
    #--   create interpolators for rain and temp
    #--   create interpolations for all time in new_time
    #--   write to nc-files named according to outputpat
    #--   (the strin "###" in outputpat is replaced by the current time value)
    #--
    def doTimeInterpolationSingle(self, interp_func, target_time):
        val_cur = np.array([])
        # print("target-times: "+str(target_time))
        if self.bPronto:
            val_cur = interp_func(target_time)
        else:
            print("ClimateINterpolator not initialized")
        #-- end if
        return val_cur
...
if __name__ == '__main__':
    ...
    if (len(argv) > 5):
        try:
            ...
            print("time [%s] must be a float" % argv[4])
        else:
            try:
                cli = ClimateInterpolator(argv[1], argv[2])
            except QHGError as e:
                print("Constructor failed: %s" % e)
            else:
                qLon, qLat = grid_utils.extractQDFCoordinates(argv[5])
                values = None
                if (argv[3] == "temp"):
                    values = cli.doInterpolationTemp(t, qLon, qLat)
                    arrays=['ActualTemps', 'AnnualMeanTemp'];
                elif (argv[3] == "rain"):
                    values = cli.doInterpolationRain(t, qLon, qLat)
                    arrays=['ActualRains', 'AnnualRainfall'];
                else:
                    print("unknown type: [%s]"%argv[3])
                #-- end if


                iResult = 0
                for arr in arrays:
                    print("doing [%s:%s]"%(argv[5], arr))
                    iResult = grid_utils.replaceQDFFileArray(argv[5], 'Climate', arr, values)
                    if (iResult != 0):
                        print("error replacing array: [%s]"%arr)
                    #--  end if
                #-- end for
        ...
        print("usage")
        print("  %s <nc_dir> <nc_pat> <what> <time> <qdf_geo>" % argv[0])
    #-- end if
#-- end main
