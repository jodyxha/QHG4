...
//----------------------------------------------------------------------------
// getHeader
//   not used anymore - check BinPheneFIle
//   
int getHeader(const char *pFile, sHeaderData *pHD) {
	...
	sMagic[4] = '\0';
	    ...
	    iResult = 0;
	    bool bNew = true;
            if (memcmp(sMagic, "PHNZ", 4) == 0) {
	        fprintf(stderr, "have header PHNZ\n");
	        bNew = true; 
            } else if (memcmp(sMagic, "PHNY", 4) == 0) {
	        fprintf(stderr, "have header PHNY\n");
	        bNew = false;
            } else {
	        iResult = -1;
	    ...
	    if (iResult == 0) {
	        memcpy(pHD->sMagic, "PHNZ", 4);

                int iHeaderLen = (bNew?4:3)*sizeof(int)+sizeof(bool);
		    ...
		    if (bNew) {
                        p = getMem(&pHD->iPloidy,    p, sizeof(int));
			printf("read ploidy %d\n", pHD->iPloidy);
		    } else {
		        pHD->iPloidy = 1;
			printf("No ploidy -> usinIG 1\n");
		    }
...
sHeaderData *checkHeaders(bpfvec &vBinPheneFiles) {
    ...
    for (uint i = 0; (iResult == 0) && (i < vBinPheneFiles.size()); i++) {
        if (vBinPheneFiles[i] != NULL) {
            ...
            sHD.iPloidy      = vBinPheneFiles[i]->getPloidy();
            sHD.bFull        = vBinPheneFiles[i]->getFull();
	    printf("have header [%s] phsize %d, numph %d, numloc %d, ploidy %d, full %s\n", sHD.sMagic, sHD.iPhenomeSize, sHD.iNumPhenomes, sHD.iNumLocs, sHD.iPloidy, sHD.bFull?"yes":"no"); 
    ...
    if (iResult == 0) {
        ...
        for (uint i = 1; (iResult == 0) && (i < vHeaders.size()); i++) {
            if ((memcmp(vHeaders[0].sMagic, vHeaders[i].sMagic, 4) != 0) ||
                ...
                (vHeaders[0].iPloidy      != vHeaders[i].iPloidy) ||
        ...
        if (iResult == 0) {
            ...
            pHD->iPloidy      = vHeaders[0].iPloidy;
            char sMagic[5];
