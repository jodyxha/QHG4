...
//
GenomeVarChecker *GenomeVarChecker::createInstance(const std::string sQDFPopFile, 
    ...
    int iResult = pGVC->init_qdf(sQDFPopFile, sSpeciesName, sAttrGenomeSize, sAttrBitsPerNuc, sDataSetGenome);
    if (iResult != 0) {
        delete pGVC;
        pGVC = NULL;
    }
...
//
GenomeVarChecker *GenomeVarChecker::createInstance(const std::string sBinGeneFile,
                                                   int iBufSizeGenomes) {

    GenomeVarChecker *pGVC = new GenomeVarChecker(iBufSizeGenomes);
    int iResult = pGVC->init_bin(sBinGeneFile);
    if (iResult != 0) {
        delete pGVC;
        pGVC = NULL;
    }
...
// init_qdf
//
int GenomeVarChecker::init_qdf(const std::string sQDFPopFile, 
                               const std::string sSpeciesName, 
                               const std::string sAttrGenomeSize,
                               const std::string sAttrBitsPerNuc,
                               const std::string sDataSetGenome) {
    ...
    if (!sAttrGenomeSize.empty()) {

        std::string sPopName = "";
        if (!sSpeciesName.empty()) {
            sPopName = qdf_getFirstPopulation(sQDFPopFile);
        } else {
            sPopName = qdf_checkForPop(sQDFPopFile, sSpeciesName);
        }
        if (!sPopName.empty()) {
            ...
            m_hSpecies     = qdf_openGroup(m_hPopulation, sPopName);
            iResult = qdf_extractAttribute(m_hSpecies, sAttrGenomeSize, 1, &m_iGenomeSize);
            if (iResult == 0) {
                
                iResult = qdf_extractAttribute(m_hSpecies, sAttrBitsPerNuc, 1, &m_iBitsPerNuc);
                if (iResult != 0) {
...
// init_bin
//
int GenomeVarChecker::init_bin(const std::string sBinGeneFile) {


    int iResult = -1;

    m_iNumGenomes = -1;
    BinGeneFile *pBG = BinGeneFile::createInstance(sBinGeneFile);
    if (pBG != NULL) {
...
// processGenomesPar
//
int GenomeVarChecker::processGenomesPar(const std::string sDataSetGenome) {
    int iResult = 0;
    ...
    hid_t hDataSet = H5Dopen2(m_hSpecies, sDataSetGenome.c_str(), H5P_DEFAULT);
