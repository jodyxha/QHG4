...
// createInstance
//
IDSampler *IDSampler::createInstance(const std::string sQDFGrid) {
    IDSampler *pIS = new IDSampler();
    int iResult = pIS->init(sQDFGrid);
    if (iResult != 0) {
        delete pIS;
        pIS = NULL;
    }
...
// init
//
int IDSampler::init(const std::string sQDFGrid) {
    int iResult = -1;
...
// getSamples
//
int IDSampler::getSamplesP(const std::string sQDFTime, const std::string sPopName, const std::string sLocFile, 
    ...
    stringvec vNames;
    if (iResult == 0) {
        iResult = fillLocData(sLocFile, vNames, mLocData);
    }
...
// getSamples
//
int IDSampler::getSamples(const std::string sQDFTime, const std::string sPopName, const std::string sLocFile, 
                          locdata  &mLocData, stringvidmap &mvIDs, idagdatamap &mAgentData) {
    int iResult = -1;
...
int IDSampler::getSamples(stringvec &vQDFPops, const std::string sPopName, const std::string sLocFile, locdata &mLocData, loctimeids &mmvIDs, idagdatamap &mAgentData, bool bParallel) {
    ...
    for (uint i = 0; (iResult == 0) && (i < vQDFPops.size()); ++i) {
        ...
        if (bParallel) {
            ...
            iResult = getSamples(vQDFPops[i], sPopName, sLocFile, mLocData, mvIDs, mAgentData);
        }
        if (iResult == 0) {
...
// 
int IDSampler::getAttributes(const std::string sQDFTime, const std::string sPopName, const std::string sLocFile, 
...
//   get attributes (mAgentData) from pQDFTime for selected IDs
// 
int IDSampler::getAttributes(stringvec &vQDFPops, const std::string sPopName, const std::string sLocFile, locdata  &mLocData, loctimeids &mmvIDs, idagdatamap &mAgentData, idset &sSelected) {
    int iResult = 0;
    for (uint i = 0; (iResult == 0) && (i < vQDFPops.size()); ++i) {
        stringvidmap mvIDs;
        
        iResult = getAttributes(vQDFPops[i], sPopName, sLocFile, mLocData, mvIDs, mAgentData, sSelected);
...
// readArrays
//
int IDSampler::readArrays(const std::string sQDFTime, const std::string sPopName) {
    int iResult = -1;
...
// fillCoordMap
//
int IDSampler::fillCoordMap(const std::string sQDFGeoGrid) {
    int iResult = -1;
    uint iNumCells = 0;
    int *pCellIDs = NULL;
    double *pdLon = NULL;
...
// fillLocData
//
int IDSampler::fillLocData(const std::string sLocFile, stringvec &vNames, locdata &mLocData) {
    int iResult = -1;
    
    LineReader *pLR = LineReader_std::createInstance(sLocFile, "rt");
    if (pLR != NULL) {
