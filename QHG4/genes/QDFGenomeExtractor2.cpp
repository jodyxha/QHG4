...
QDFGenomeExtractor2 *QDFGenomeExtractor2::createInstance(const char *pQDFGeoFile,
                                                         ...
                                                         WELL512 *pWELL,
                                                         bool bCartesian) {
    QDFGenomeExtractor2 *pQPC = new QDFGenomeExtractor2(pWELL, bCartesian);  // num parents
...
int QDFGenomeExtractor2::extractAdditionalAttributes() {
    ...
    // our genome simulation has a ploidy of 2
    m_iPloidy = 2;
...
// calcNumItemsInBlock
//
int QDFGenomeExtractor2::calcNumItemsInBlock() {
    int iNumItems = 0;
    if (m_pAttrBitsPerNuc != NULL) {
        if (m_iBitsPerNuc == BitGeneUtils::BITSINNUC) {
            iNumItems = BitGeneUtils::NUCSINBLOCK;
        } else {
            iNumItems = GeneUtils::NUCSINBLOCK;
        }	
    } else {
        m_iBitsPerNuc = GeneUtils::BITSINNUC;
        iNumItems = GeneUtils::NUCSINBLOCK;
...
hid_t QDFGenomeExtractor2::readQDFSequenceSlab(hid_t hDataSet, 
                                               ...
                                               hid_t hDataSpace, 
                                               ulong *aBuf) {
    return  H5Dread(hDataSet, H5T_NATIVE_ULONG, hMemSpace, hDataSpace, H5P_DEFAULT, aBuf);
}
