...
// createInstance
//
AlleleCounter *AlleleCounter::createInstance(const std::string sQDFPopFile, const std::string sSpeciesName) {
    AlleleCounter *pNC = new AlleleCounter();
    int iResult = pNC->init(sQDFPopFile, sSpeciesName);
    if (iResult != 0) {
        delete pNC;
        pNC = NULL;
    }
...
// init
//
int AlleleCounter::init(const std::string sQDFPopFile, const std::string sSpeciesName) {
    int iResult = -1;
...
int AlleleCounter::selectIndexes(int iSelectionType) {
    ...
    if (pQA != NULL) {
        iResult = pQA->openArray(POPGROUP_NAME, m_sPopName, POP_DS_AGENTS);
