...
BinPheneFile *BinPheneFile::createInstance(const std::string sFile) {
    BinPheneFile *pBG = new BinPheneFile();
    int iResult = pBG->init(sFile);
    if (iResult != 0) {
        delete pBG;
        pBG = NULL;
    }
...
// readHeader
//   
int BinPheneFile::init(const std::string sFile) {
    int iResult = 0;
    m_fIn = fopen(sFile.c_str(), "rb");
    if (m_fIn != NULL) {
        m_sCurName = sFile;
        iResult = readHeader();
    } else {
    ...
    if (m_fIn != NULL) {
        ...
        if (iRead == 1) {
            ...
            if ((memcmp(m_sMagic, "PHNY", 4) == 0) ||
                (memcmp(m_sMagic, "PHNZ", 4) == 0)) {
                if (memcmp(m_sMagic, "PHNY", 4) == 0) {
                    m_bExtended = false;
                    m_iPloidy   = 1;
                } else if (memcmp(m_sMagic, "PHNZ", 4) == 0) {
                    m_bExtended = true;
                }


                int iHeaderLen = 4*sizeof(int)+sizeof(bool);
                ...
                if (iRead == 1) {
                    ...
                    if (m_bExtended) {
                        p = getMem(&m_iPloidy,    p, sizeof(int));
                    }
                    p = getMem(&m_bFull,       p, sizeof(bool));
                    m_iNumBlocks = m_iPloidy*m_iPhenomeSize;
...
int BinPheneFile::readPhenome() {
    ...
    if (m_fIn != NULL) {
        ...
        float *pPh =  new float[m_iNumBlocks];

        int iRead = fread(pPh, sizeof(float), m_iNumBlocks, m_fIn);

        if (iRead == m_iNumBlocks) {
...
// getMapsAll
//   
void BinPheneFile::getMapsAll(id_phenomes &mIDPhen, tnamed_ids &mvIDs, id_node &mIDNodes, id_locs &mIdLocs, named_locs &mNamedLocs) {
    mIDPhen    = m_mIDPhen;
    mvIDs      = m_mvIDs;
    mIDNodes   = m_mIDNodes;
    mIdLocs    = m_mIdLocs;
...
// getMapsMain
//   
void BinPheneFile::getMapsMain(id_phenomes &mIDPhen, tnamed_ids &mvIDs) {
    mIDPhen    = m_mIDPhen;
    mvIDs      = m_mvIDs;
}
