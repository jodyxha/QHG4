...
//  constructor
//
HistoMaker_multi::HistoMaker_multi(groupedvals<double> vGroupedVals) 
...
//  destructor
//
HistoMaker_multi::~HistoMaker_multi() {
...
//  createHistos
//
int **HistoMaker_multi::createHistos(double dMin, double dMax, int iNumBins, bool bStrict) {
    ...
    for (it = m_vGroupedVals.begin(); it != m_vGroupedVals.end(); ++it) {
        printf("[HistoMaker_multi::createHistos] histo #%d for %d...\n", iCurIndex, it->first);
        createSingleHisto(iCurIndex, it->second);
        iCurIndex++;
    }
    printf("[HistoMaker_multi::createHistos] histo created\n");
...
//  createSingleHisto
//
int HistoMaker_multi::createSingleHisto(uint iIndex, const std::vector<double> &vVals) {
    int iResult = 0;

    printf("[HistoMaker_multi::createSingleHisto] creating histo for  %u (%zd elements)\n", iIndex, vVals.size());
    ...
    printf("[HistoMaker_multi::createSingleHisto] %zd histos created\n", vVals.size());
...
// showHistos
//
void HistoMaker_multi::showHistos(maphistos &mH) {
