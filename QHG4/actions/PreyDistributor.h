...
public:
    ...
    // in preloop:
    int registerPredator(const std::string sPredName);

    // in initializeStep
    int buildAssignments(const std::string sPredName, float fTime);

    // mainly debugging
    void showRelations();
    void showFrequencies();
    void showAssignments();
    void showAgentAssignments(int iCellID, const std::string sPredName, int iAgentIndex);
    /*    void showUnmatched();*/
protected:
    int getFrequencies();
    int calcAssignments();
