...
//
template<typename T>
RandPermPair<T>::RandPermPair(SPopulation<T> *pPop, SCellGrid *pCG, std::string sID, WELL512 **apWELL) 
    : Action<T>(pPop,pCG,ATTR_RANDPERMPAIR_NAME,sID),
      m_apWELL(apWELL) {
...
//
template<typename T>
bool RandPermPair<T>::isEqual(Action<T> *pAction, bool bStrict) {
    return true;
}
