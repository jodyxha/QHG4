...
template<typename T>
const char *NPersAHybLinearBirthRel<T>::asNames[] = {
    ATTR_NPERSAHYBLINBIRTHREL_HYBMINPROB_NAME,
};
...
//
template<typename T>
NPersAHybLinearBirthRel<T>::NPersAHybLinearBirthRel(SPopulation<T> *pPop, SCellGrid *pCG, std::string sID, WELL512 **apWELL, int **aiNumBirths) 
    : Action<T>(pPop,pCG,ATTR_NPERSAHYBLINBIRTHREL_NAME,sID),
      m_apWELL(apWELL),
      m_dHybMinProb(1.0),
...
//
template<typename T>
NPersAHybLinearBirthRel<T>::NPersAHybLinearBirthRel(SPopulation<T> *pPop, SCellGrid *pCG, std::string sID, WELL512 **apWELL, int **aiNumBirths, double dHybMinProb) 
    : Action<T>(pPop,pCG,ATTR_NPERSAHYBLINBIRTHREL_NAME,sID),
      m_apWELL(apWELL),
...
//
template<typename T>
NPersAHybLinearBirthRel<T>::~NPersAHybLinearBirthRel() {
...
//
template<typename T>
int NPersAHybLinearBirthRel<T>::preLoop() {
    int iResult = -1;
...
//
template<typename T>
int NPersAHybLinearBirthRel<T>::postLoop() {
    int iResult = 0;
...
//
template<typename T>
int NPersAHybLinearBirthRel<T>::initialize(float fT) {
...
//
template<typename T>
int NPersAHybLinearBirthRel<T>::execute(int iAgentIndex, float fT) {
    ...
    if (paM->m_iLifeState > 0) {
    	...
    	int iMateIndex = paM->m_iMateIndex;
        ...
        if (paM->m_dCC > 0) {
            ...
            if (paM->m_dBReal > 0) { // positive birth prob
                if ((paM->m_iGender == 0) && (iMateIndex >= 0)) {
                    // get the father
                    T *paP =  &(this->m_pPop->m_aAgents[iMateIndex]);
                    // parent fertility factors

                    float fFertM = m_dHybMinProb + 2*(1-m_dHybMinProb)*fabs(paM->m_fNeaFrac-0.5);
                    float fFertP = m_dHybMinProb + 2*(1-m_dHybMinProb)*fabs(paP->m_fNeaFrac-0.5);
...
//
template<typename T>
int NPersAHybLinearBirthRel<T>::finalize(float fT) {
...
//
template<typename T>
int NPersAHybLinearBirthRel<T>::extractAttributesQDF(hid_t hSpeciesGroup) {
    
    int iResult = 0;

    
    if (iResult == 0) {
        iResult = qdf_extractAttribute(hSpeciesGroup, ATTR_NPERSAHYBLINBIRTHREL_HYBMINPROB_NAME, 1, &m_dHybMinProb);
        if (iResult != 0) {
            LOG_ERROR("[NPersAHybLinearBirthRel] couldn't read attribute [%s]", ATTR_NPERSAHYBLINBIRTHREL_HYBMINPROB_NAME);
        }
    }
...
//
template<typename T>
int NPersAHybLinearBirthRel<T>::writeAttributesQDF(hid_t hSpeciesGroup) {
    
    int iResult = 0;

    iResult += qdf_insertAttribute(hSpeciesGroup, ATTR_NPERSAHYBLINBIRTHREL_HYBMINPROB_NAME, 1, &m_dHybMinProb);
...
//
template<typename T>
int NPersAHybLinearBirthRel<T>::modifyAttributes(const std::string sAttrName, double dValue) {
    int iResult = 0;
    if (sAttrName == ATTR_NPERSAHYBLINBIRTHREL_HYBMINPROB_NAME) {
...
//
template<typename T>
int NPersAHybLinearBirthRel<T>::tryGetAttributes(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getAttributes();
    if (this->checkAttributes(mParams) == 0) {
        iResult = 0;
        iResult += getAttributeVal(mParams, ATTR_NPERSAHYBLINBIRTHREL_HYBMINPROB_NAME, &m_dHybMinProb);
    }
    return iResult;
}
...
//
template<typename T>
bool NPersAHybLinearBirthRel<T>::isEqual(Action<T> *pAction, bool bStrict) {
    bool bEqual = false;
    NPersAHybLinearBirthRel<T>* pA = static_cast<NPersAHybLinearBirthRel<T>*>(pAction);
