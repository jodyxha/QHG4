...
int checkArrayName(arrind &aiNameIndex, array_data &mvArrayData) {
    ...
    if (iCode != ARR_CODE_NONE) {
        xha_printf("added ARR [%s@%d] -> Group [%s], dataset [%s], type %d\n", 
               ...
               mvArrayData[aiNameIndex].sDataSet, 
...
int checkConsistent(string_list &vQDFs, array_data &mvArrayData, which_data &mvWhich) {
    ...
    hid_t  hQDFGeoGrid = qdf_openFile(vQDFs[0]);
    if (hQDFGeoGrid != H5P_DEFAULT) {
        ...
        for (it = mvArrayData.begin(); (iResult == 0) && (it != mvArrayData.end()); ++it) {
            ...
            for (uint i = 0; bSearching && (i < vQDFs.size()); i++) {
                hid_t hQDFData = qdf_openFile(vQDFs[i]);
...
// extractData
//
double *extractData(const std::string sQDF, int iNumCells, const ds_info &pGroupDS) {
    int iResult = -1;
    double *pdArr  = NULL;
    char   *pdArr1 = NULL;
    ...
    case DS_TYPE_DOUBLE: 
        iResult = qdf_readArray(hGroup, pGroupDS.sDataSet, iNumCells, (double *)pdArr);
        break;

    case DS_TYPE_CHAR:
        pdArr1 = new char[iNumCells];
        iResult = qdf_readArray(hGroup, pGroupDS.sDataSet, iNumCells, (char *)pdArr1);
        if (iResult == 0) {
...
// replacePat
//
std::string replacePat(std::string sPat, const std::string &sReplace) {
...
// writePNGFile
//
int writePNGFile(uchar **ppuData, int iW, int iH, const std::string sOut) {
    int iResult = -1;
