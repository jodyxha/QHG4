...
void usage(const char *pApp) {
    ...
    printf("Usage:\n");
    printf("  %s -i <input_qdf> -d <distance> [-o <output_qdf>] [-b <Box>[,<Box>]*] [-m <bridge-file>]\n", pApp);
    printf("where\n");
    ...
    printf("  bridge-file   a text file with containing the node ids of the endpoints of the bridges (1 per line)\n"); 
    printf("\n");
}
    ...
    if (pBoxes != NULL) {
        ...
        while ((iResult == 0) && (pBox != NULL)) {
            ...
            if (p != NULL) {
                ...
                printf("expecetd ':'-separated coordinate list: [%s]\n", sBox);
            }
...
// readBridges
//
int readBridges(const char *pBridgeFile, bridgelist &vBridges) {
    int iResult = -1;

    LineReader *pLR = LineReader_std::createInstance(pBridgeFile, "rt");
    if (pLR != NULL) {
        iResult = 0;
        ...
        while ((iResult == 0) && (pLine != NULL) && !pLR->isEoF()) {
            ...
            if (p1 != NULL) {
                ...
                char *p2 = strtok(NULL, " :;.t"); 
                if (p2 != NULL) {
                    int iFrom;
                    int iTo;
                    if (strToNum(p1, &iFrom)) {
                        if (strToNum(p2, &iTo)) {
                            vBridges.push_back(bridgedef(iFrom, iTo));
                            iResult = 0;
                            pLine = pLR->getNextLine();
                        } else {
                            printf("not a number: [%s]n", p2);
                        }
                    } else {
                        printf("not a number: [%s]n", p1);
                    }
                } else {
...
int writeAllToQDF(SCellGrid *pCG, const char *sOutputQDF, cellset &sAll) {
    ...
    hid_t hFile = qdf_createFile(sOutputQDF, 0, 0.0, "SeaWays");
...
// writeAllToQDF
//
int writeAllToQDF(SCellGrid *pCG, const char *sOutputQDF, const distancemap &mFinalDistances, double dSampleDist, bridgelist &vBridges) {
    int iResult = 0;
...
// writeAllToQDFIn
//
int writeAllToQDFIn(SCellGrid *pCG, const char *sOutputQDF, const distancemap &mFinalDistances, double dSampleDist, bridgelist &vBridges) {
    int iResult = 0;
                    ...
                    if  (iResult == 0) {
                        ...
                        if (pCD != NULL) {
                            ...
                            if (pSCM != NULL) {
                                ...
                                bridgelist vBridges;
                                if (sBridges != NULL) {
                                    readBridges(sBridges, vBridges);
                                }

                                if (sOutputQDF != NULL) {
                                    writeAllToQDF(pCG, sOutputQDF, mFinal, dDistance, vBridges);
                                } else {
                                    writeAllToQDFIn(pCG, sInputQDF, mFinal, dDistance, vBridges);
                                }
                                /*
