...
bool PNGImage::createPNGFromData(double ***aadData, const std::string sOutputFile) {
    ...
    if (m_bInitialized) {
        ...
        if (bOK) {
            bOK = writePNGFile(sOutputFile);
        }   
    }
    return bOK;
}
   
bool PNGImage::createPNGFromData(uchar **aaucData, const std::string sOutputFile) {
    bool bOK = false;
    if (m_bInitialized) {
...
// writePNGFile
//
bool PNGImage::writePNGFile(const std::string sOutputFile) {
    bool bOK = true;
...
// readPNGFile
//
bool PNGImage::readPNGFile(const std::string sInputFile) {
    bool bOK = true;
	uchar sHeader[HEADER_SIZE];	// 8 is the maximum size that can be checked

	/* open file and test for it being a png */
	FILE *fp = fopen(sInputFile.c_str(), "rb");
	if (fp != NULL) {
