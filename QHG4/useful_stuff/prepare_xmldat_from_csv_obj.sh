...
then 
    ...
    if [ "X${csv_ignore}" == "Xdefault" ]
    then 
       csv_ignore="maternal_sdev_hyb_sapiens:paternal_sdev_hyb_sapiens:NPPCap_efficiency_sapiens:NPPCap_NPP_min_sapiens:maternal_sdev_hyb_neander:paternal_sdev_hyb_neander:NPPCap_efficiency_neander:Navigate_decay"
    fi
    if [ "X${csv_ignore}" != "X" ]
    then
    ...
    if [ "X${xml_block}" == "Xdefault" ]
    then
        xml_block="NPPCap_K_min_sapiens:NPPCap_NPP_min_sapiens"
    fi
    if [ "X${xml_block}" != "X" ]
    then
...
then
    ...
    ${QHG4_DIR}/useful_stuff/csv2xmldat_obj.py  -w ${width} -x ${xml_template_nea} -d ${dat_template_nea} -c ${csv_in} -o ${temp_dir}/temp_${out_body}_nea ${xml_block} ${csv_ignore} -s neander
    if [ $? -eq 0 ]
    then
      # we dont't need to do the xml, because they would be the same we already have ; will return -1 if bad sep
      ${QHG4_DIR}/useful_stuff/csv2xmldat_obj.py  -w ${width} -a dat -x ${xml_template_sap} -d ${dat_template_sap} -c ${csv_in} -o ${temp_dir}/temp_${out_body}_sap ${xml_block} ${csv_ignore} -s sapiens
      if [ $? -eq 0 ]
      then
...
#          mv temp_${out_body}_nea_${ii}.xml  ${out_dir2}/${out_body}${ii}_sap.xml
  ...
  echo "   xml_tmpl_nea=<nea-xml-template-file>"
  echo "   xml_tmpl_sap=<sap-xml-template-file>"
  echo "   dat_tmpl_nea=<nea-dat-template-file>"
  echo "   dat_tmpl_sap=<sap-dat-template-file>"
  echo "   outsub=<output_subdir>"
  echo "   xml_block=<param-name>[:<param-name>]*"
  echo "   csv-ignore=<param-name>[:<param-name>]*"
  echo ""
  echo "Example:"
  echo "  $0  shorty.csv hyb_nolog_small_iter_1kB 5 \\"
  echo "      xml_tmpl_nea=template_file_nea.xml  xml_tmpl_sap=template_file_sap.xml \\"
  echo "      dat_tmpl_nea=template_file_nea.dat  dat_tmpl_sap=template_file_sap.dat \\"
  echo "      outsub=hyb_nolog_small_iter_1kB \\"
  echo "      xml_block=NPPCap_K_min_sapiens:NPPCap_NPP_min_sapiens \\"
  echo "      csv_ignore=maternal_sdev_hyb_sapiens:paternal_sdev_hyb_sapiens:NPPCap_efficiency_sapiens:NPPCap_NPP_min_sapiens:maternal_sdev_hyb_neander:paternal_sdev_hyb_neander:NPPCap_efficiency_neander:Navigate_decay"
fi
