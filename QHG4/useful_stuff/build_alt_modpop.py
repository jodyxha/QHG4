...
popfiles = [
    ...
    qhg_base + '/populations/configure.sh',
    ]
...
if len(argv) > 2:
    ...
    prefix  = argv[1]
    # create all subdirectories necessary
    pos = prefix.rfind('/')
    if (pos >= 0):
        prefix_dir = prefix[0:pos]
        if not os.path.isdir(prefix_dir) and not os.path.isfile(prefix_dir):
    ...
    try:
        # can't create a dir with the same pathname as file 
        if (not os.path.isfile(prefix)):
            ...
            print("ERROR: There already is a file with name [%s]"%prefix)
        #-- end if
    except IOError as e:
