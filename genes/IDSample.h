...
public:
    ...
    void addAgentData(const std::string sLocation, float fTime, agdata *pAD);

    void addAgentDataVec(const std::string sLocation, float fTime, std::vector<agdata*> &vAGD);
    ...
    int write(const std::string sOutputFile);    
    int read(const std::string sInPutFile);
    void display();
private:
