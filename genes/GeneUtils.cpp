...
//double GeneUtils::dT1 = 0;
//double GeneUtils::dT2 = 0;
//----------------------------------------------------------------------------
// bitcount
//   (from http://en.wikipedia.org/wiki/Hamming_weight)
//
uint GeneUtils::bitcount(ulong x) {
...
//
void GeneUtils::crossOver(ulong *pGenome1, const ulong *pGenome2, int iGenomeSize, int iNumCrossOvers, WELL512* pWELL) {
...
//----------------------------------------------------------------------------
// makeBlockBreaks
//   creates bloc breakpoints for crossover2
//
void GeneUtils::makeBlockBreaks(blockbreak_t &vBlockBreaks, int iNumBlocks, int iNumCrossOvers, WELL512* pWELL) {
    uint iNumBits = iNumBlocks*BITSINBLOCK;
...
//   mixes the two chromosomes of pGenome2 into the two chromosomes of pGenome1
// 
void GeneUtils::crossOver2(ulong *pGenome1, const ulong *pGenome2, uint iNumBlocks, blockbreak_t &vBlockBreaks) {
    ...
    blockbreak_t::const_iterator itt;
    for (itt = vBlockBreaks.begin(); itt != vBlockBreaks.end(); itt++) {
        uint iBlock = itt->first;
        iCurChromosome =iS%2;
...
//
void GeneUtils::freeReco(ulong *pGenome1, ulong *pGenome2, int iNumBlocks, WELL512* pWELL) {
...
//  we assume pWELL is not NULL
//
ulong *GeneUtils::makeFreeMasks(ulong *pMasks, int iNumBlocks, WELL512 *pWELL) {
    ulong lV = 0;
...
//   completely random mixture of the two chromosomes of pGenome2 into the 
//   two chromosomes of pGenome1
//   for a given array of masks
// 
void GeneUtils::freeReco2(ulong *pGenome1, ulong *pGenome2, ulong *pMasks, int iNumBlocks) {
    //    int iNumBlocks = numNucs2Blocks(iGenomeSize);

    for (int iBlock = 0; iBlock < iNumBlocks; iBlock++) {

        ulong lLeft  = pMasks[iBlock];
        ulong lRight = ~lLeft;
...
// fillConstValues
//
ulong *GeneUtils::fillConstValues(int iNumParents, int iGenomeSize, ulong iValue) {
    int iNumBlocks = numNucs2Blocks(iGenomeSize);

    ulong *pBuf = new ulong[iNumParents*iNumBlocks];
    if (iValue > 0) {
        iValue = hff; //all ones
    }
    for (int i = 0; i < iNumParents*iNumBlocks; i++) {
        pBuf[i] = iValue;
    }
    return pBuf;
}
...
// writePlinkMapFile
//
int GeneUtils::writePlinkMapFile(const std::string sOut, int iGenomeSize) {
    int iResult = -1;
