...
//    m_dRecycleDeadTime = 0;
    ...
    m_adCapacities  = new double[m_iNumCells];
    memset(m_adCapacities, 0, m_iNumCells*sizeof(double));
...
int SPopulation<T>::finalizeStep() {    
    ...
    if (iResult == 0) {
        iResult = m_pMoveStats->insertNewStats(m_fCurTime);
    }
...
//  This is ok to do here because this is not a parallel section
...
int SPopulation<T>::moveAgent(int iCellIndexFrom, int iAgentIndex, int iCellIndexTo) {
    ...
    makePopSpeficiMove(iCellIndexFrom, iAgentIndex, iCellIndexTo);
    
    // do arrival hops/dist/time
    if  ((m_pCG->m_pGeography != NULL) && (m_pMoveStats != NULL)) {
        
       //@@@ MoveStat new begin
        if ( m_pMoveStats->m_aiHops[iCellIndexTo] < 0) {
            double dDist = m_pMoveStats->m_adDist[iCellIndexFrom];
            ...
            int    iNewHops = m_pMoveStats->m_aiHops[iCellIndexFrom]+1;
            ...
            m_pMoveStats->addStatCandidates(iCellIndexTo, dNewDist, iNewHops, dNewTime);
        }
    }
    return iResult;
}
...
int SPopulation<T>::readSpeciesData(ParamProvider2 *pPP) {
    ...
    if (iResult == 0) {
        // read action params
        iResult = m_prio.getActionParams(pPP->getClassInfo()->mods);
...
int  SPopulation<T>::addAgent(int iCellIndex, char *pData, bool bUpdateCounts) {
    ...
    if (m_pMoveStats != NULL) {
        if (m_pMoveStats->m_aiHops[iCellIndex] < 0) {
            m_pMoveStats->m_adDist[iCellIndex] = 0;
            m_pMoveStats->m_adTime[iCellIndex] = 0;
        }
    }
...
//
template<typename T>
int  SPopulation<T>::shiftIDs(idtype iOffset) {
    for (int iA = getFirstAgentIndex(); iA <= getLastAgentIndex(); iA++) {
...
//
template<typename T>
int SPopulation<T>::modifyParams(const std::string sAttrName, double dValue) {
    
    int iResult = m_prio.modifyParams(sAttrName, dValue);
    return iResult; 
}	
...
int  SPopulation<T>::writeSpeciesDataQDF(hid_t hSpeciesGroup) {
    ...
    if (iResult == 0) {
        iResult = m_prio.writeActionParamsQDF(hSpeciesGroup, m_iQDFVersionOut);
    }
...
int  SPopulation<T>::readSpeciesDataQDF(hid_t hSpeciesGroup) {
    ...
    if (iResult == 0) {
        iResult = m_prio.extractActionParamsQDF(hSpeciesGroup, m_iQDFVersionIn);
    }
...
//
template<typename T>
int  SPopulation<T>::writeStatArrsQDF(hid_t hSpeciesGroup) {
    int iResult = 0;
    if (iResult == 0) {
        iResult = qdf_writeArray(hSpeciesGroup, SPOP_DS_CAP, m_iNumCells, m_adCapacities);
    }
    if (iResult == 0) {
        iResult = m_pMoveStats->writeDataQDF(hSpeciesGroup);
    }
    return iResult;
}
...
//
template<typename T>
int  SPopulation<T>::readStatArrsQDF(hid_t hSpeciesGroup) {
    int iResult = 0;

    // IMPORTANT: NO error if arrays don't exist
    //             warning messages are sufficient.
    // Actually we don't need to read capacities - they will be calculated
    /*
    if (iResult == 0) {
        iResult = qdf_readArray(hSpeciesGroup, SPC_CAP, m_iNumCells, m_adCapacities);
    }
    */
    if (iResult == 0) {
        iResult = m_pMoveStats->readDataQDF(hSpeciesGroup);
    }
...
// dumpAdditionalDataQDF
//  write stat arrays:
//  m_adCapacity + movstat arrays
//
template<typename T>
int  SPopulation<T>::dumpStatArrsQDF(hid_t hSpeciesGroup) {
      
    return this->writeStatArrsQDF(hSpeciesGroup);
}
...
//
template<typename T>
int  SPopulation<T>::restoreStatArrsQDF(hid_t hSpeciesGroup) {
    return this->readStatArrsQDF(hSpeciesGroup);
}
