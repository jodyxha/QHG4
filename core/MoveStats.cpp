...
// readData
//
int MoveStats::readDataQDF(hid_t hGroup) {
    int iResult = 0;
...
// write
//
int MoveStats::writeDataQDF(hid_t hGroup) {
    int iResult = 0;
     
    if (iResult == 0) {
        printf("[ MoveStats::writeDataQDF] writing Hops[%d]\n", m_iNumCells);
        iResult = qdf_writeArray(hGroup, SPOP_DS_HOPS, m_iNumCells, m_aiHops);
    }
            
    if (iResult == 0) {
        printf("[ MoveStats::writeDataQDF] writing Dist[%d]\n", m_iNumCells);
        iResult = qdf_writeArray(hGroup, SPOP_DS_DIST, m_iNumCells, m_adDist);
    }
            
    if (iResult == 0) {
        printf("[ MoveStats::writeDataQDF] writing Time[%d]\n", m_iNumCells);
