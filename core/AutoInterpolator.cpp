...
int AutoInterpolator::prepareTargets() {
    ...
    for (uint i = 0; (iResult == 0) && (i < m_vTargets.size()); i++) {
        ...
        const std::string &sFullName = m_vTargets[i].first;
        const std::string &sGroup    = m_vTargets[i].second.first;
        const std::string &sArray    = m_vTargets[i].second.second;
        
        if ((sGroup == GEOGROUP_NAME) || (sGroup == "geo")){
            if (sArray == GEO_DS_ALTITUDE)  {
                // make sure group name is not 'shortcut'
                m_vTargets[i].second.first = GEOGROUP_NAME;
                // set event
            ...
            if (sArray == VEG_DS_NPP)  {
                // make sure group name is not 'shortcut'
                m_vTargets[i].second.first = VEGGROUP_NAME;
                // set event
...
void AutoInterpolator::allocateArrays() {
    for (uint k = 0; k < m_vTargets.size(); k++) {
        ...
        const std::string &sFullName =  m_vTargets[k].first;
        for (int i = 0; i < 2; i++) {
...
// loadArray
//
int AutoInterpolator::loadArray(hid_t hFile, int iWhich, string_combi &sTarget) {
    int iResult = -1;

    // get new array size
    hsize_t dims;

    const std::string &sFullName  = sTarget.first;
    const std::string &sGroupName = sTarget.second.first;
    const std::string &sArrayName = sTarget.second.second;
...
// loadArrayForTime
//
int AutoInterpolator::loadArrayForTime(int iTime, int iWhich, string_combi &sTarget) {
    int iResult = -1;
...
// loadArrayFromGrid
//
int AutoInterpolator::loadArrayFromGrid(string_combi &sTarget) {
    int iResult = 0;
...
int AutoInterpolator::interpolate(int iSteps) {
    ...
    if (iSteps == 1) {
        for (string_combi sc : m_vTargets) {
            double *pSource = m_mDiff[sc.first].second;
            double *pTarget = m_mTargets[sc.first].second;
...
int AutoInterpolator::startInterpolations(int iFirstStep) {
    ...
    if (iFirstStep <=  m_mFileList.begin()->first) {
        ...
        if (itC != m_mFileList.end()) {
            ...
            for (uint i = 0; (iResult == 0) && (i < m_vTargets.size()); i++) {
                stdprintf("loading array [%s]\n", m_vTargets[i].first); fflush(stdout);
...
int AutoInterpolator::calcNextDiff() {
    ...
    if (m_itCur != m_mFileList.end()) {
        ...
        for (uint i = 0; (iResult == 0) && (i < m_vTargets.size()); i++) {
            ...
            std::string &sFullName = m_vTargets[i].first;
...
#pragma omp parallel for
        ...
        for (uint i = 0; (iResult == 0) && (i < m_vTargets.size()); i++) {
            const length_array &la = m_mDiff[m_vTargets[i].first];
...
int AutoInterpolator::calcDiffs(int iTime1, int iTime2) {
    ...
    for (auto tt : m_vTargets) {
        ...
        const std::string &sFullName =  tt.first;
