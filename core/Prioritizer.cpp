...
//
template<typename A>
int Prioritizer<A>::addAction(std::string name, Action<A> *act) {
    int iResult = 0;
    if (m_names.find(name) != m_names.end()) {
        iResult = -1;
        stdprintf("[Prioritizer<A>::addAction] WARNING: there already is an action with name [%s]\n", name);
    } else {
        m_names[act->getActionName()] = act;
        //m_names[name] = act;
        stdprintf("[Prioritizer<A>::addAction] Prioritizer added actions [%s]\n", name);
    }
    return iResult;
...
bool Prioritizer<A>::hasParam(std::string sAttrName) {
    ...
    for (it = m_names.begin(); (!bResult) && (it != m_names.end()); ++it) {
        bResult |= it->second->hasParam(sAttrName);
    }
...
//
template<typename A>
int Prioritizer<A>::extractActionParamsQDF(hid_t hSpeciesGroup, int iVersion) {
    int iResult = -1;
    if (iVersion >= 4) {
...
//-----------------------------------------------------------------------------
// extractActionParamsQDF4
//
template<typename A>
int Prioritizer<A>::extractActionParamsQDF4(hid_t hSpeciesGroup) {
    ...
    for (it = m_names.begin(); iResult == 0 && it != m_names.end(); ++it) {
        ...
        if (hActionGroup != H5P_DEFAULT) {
            iResult = it->second->extractParamsQDF(hActionGroup);
        } else {
            iResult = -1;
...
//
template<typename A>
int Prioritizer<A>::writeActionParamsQDF4(hid_t hSpeciesGroup) {
    ...
    for (it = m_names.begin(); iResult == 0 && it != m_names.end(); ++it) {
        ...
        if (hActionGroup != H5P_DEFAULT) {
            iResult = it->second->writeParamsQDF(hActionGroup);
...
//
template<typename A>
int Prioritizer<A>::modifyParams(const std::string sAttrName, double dValue) {
    ...
    // we cycle through all actions until the parameter has changed
    for (it = m_names.begin(); (iResult < 0) && (it != m_names.end()); ++it) {
        stdprintf("[Prioritizer<A>::modifyParams] adding parameters for action %s\n",it->first);
        iResult = it->second->modifyParams(sAttrName, dValue);
    }
...
//
template<typename A>
int Prioritizer<A>::getActionParams(const modulemap &mModules) {
    int iResult = 0;
    ...
    for (it = m_names.begin(); (iResult == 0) && (it != m_names.end()); ++it) {
        ...
        if (itM != mModules.end()) { 
            iResult = it->second->tryGetParams(itM->second);  // in Action
            if (iResult != 0) {
                stdprintf("[Prioritizer<A>::getActionParams] Couldn't get values for [%s]\n", it->first);
            }
        } else {
            if (it->second->getNumParams() != 0) {
