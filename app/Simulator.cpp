...
int Simulator::preLoop()  {
    ...
    if (isReady()) {
        ...
        for (it_pop = m_pPopLooper->getMap().begin(); (iResult == 0) && (it_pop != m_pPopLooper->getMap().end()); ++it_pop) {
            iResult = it_pop->second->preLoop();
        }
...
int Simulator::runLoop() {
    ...
    popmap::const_iterator it_pop;
    for (it_pop = m_pPopLooper->getMap().begin(); it_pop != m_pPopLooper->getMap().end(); ++it_pop) {
...
int Simulator::postLoop() {
    ...
    for (it_pop = m_pPopLooper->getMap().begin(); (iResult == 0) && (it_pop != m_pPopLooper->getMap().end()); ++it_pop) {
        ...
        iResult = it_pop->second->postLoop();
    }
...
int Simulator::handleWriteEvent(const std::string sDesc, int iDumpMode) {
            ...
            if  (iWhat != WR_NONE) {
                if (iResult == 0) {
                    ...
                    if  ((!sSub.empty()) && (iWhat >= WR_POP)) {
                        ...
                        vSubs.push_back(std::pair<std::string, int>(sSub, iWS));
                    }
    ...
    if (iResult == 0) {
        ...
        popmap::const_iterator it_pop;
        for (it_pop = m_pPopLooper->getMap().begin(); (iResult == 0) && (it_pop != m_pPopLooper->getMap().end()); ++it_pop) {
            iResult = it_pop->second->preWrite(m_iCurStep);
        }
        ...
        if (sCommand == CMD_SET_ITERS) {
            ...
            if (iNum == 4) {
                ...
                if (strToNum(sValue, &dValue)) {
                    ...
                    PopBase *pPop = m_pPopLooper->getPopByName(sPopName);
                    if (pPop != NULL) {
                        pPop->modifyParams(sParam, dValue);
                        iResult = 0;
                    } else {
...
//  write output
//
int Simulator::writeState(const std::string sQDFOut, int iWhat, std::vector<std::pair<std::string, int>> &vSub, int iDumpMode) {
    int iResult = 0;
