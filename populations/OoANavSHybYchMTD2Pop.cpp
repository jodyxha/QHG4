...
#include "NPersWeightedMove2.cpp"
...
#include "LocEnv2.cpp"
#include "PrivParamMix2.cpp"
#include "Navigate.cpp"
#include "OoANavSHybYchMTD2Pop.h"
...
// constructor
//
OoANavSHybYchMTD2Pop::OoANavSHybYchMTD2Pop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    : SPopulation<OoANavSHybYchMTD2Agent>(pCG, pPopFinder, iLayerSize, apIDG, aulState, aiSeeds),
    ...
    m_pLE = new LocEnv2<OoANavSHybYchMTD2Agent>(this, m_pCG, "", m_apWELL, m_pAgentController); 
    m_pLE->init();
    // addObserver(m_pLE);
    m_pPPM = new PrivParamMix2<OoANavSHybYchMTD2Agent>(this, m_pCG, "", m_apWELL); 
   
    m_pHybBirthDeathRel = new NPersZHybBirthDeathRel<OoANavSHybYchMTD2Agent>(this, m_pCG, "", m_apWELL, m_aiNumBirths);


    m_pWM = new NPersWeightedMove2<OoANavSHybYchMTD2Agent>(this, m_pCG, "", m_apWELL, m_pLE);

    m_pPair = new RandomPair<OoANavSHybYchMTD2Agent>(this, m_pCG, "", m_apWELL);

    m_pGO = new GetOld<OoANavSHybYchMTD2Agent>(this, m_pCG, "");

    m_pOAD = new PersOldAgeDeath<OoANavSHybYchMTD2Agent>(this, m_pCG, "", m_apWELL);

    m_pFert = new PersFertility<OoANavSHybYchMTD2Agent>(this, m_pCG, "");

    m_pNavigate = new Navigate<OoANavSHybYchMTD2Agent>(this, m_pCG, "", m_apWELL);
...
// destructor
//
OoANavSHybYchMTD2Pop::~OoANavSHybYchMTD2Pop() {
...
//
int OoANavSHybYchMTD2Pop::setParams(const std::string sParams) {
    int iResult = 0;
    if (!sParams.empty()) {
        stringvec vParts;
        ...
        if (iNum == 2) {
                    ...
                    if (iNumSub1 == 2) {
                        ...
                        if (strToNum(vSubs1[1], &dSDev)) {
                            ...
                            if (sAvgName == "MaternalContribAvg") {
                                ...
                                stdprintf("[OoANavSHybYchMTD2Pop::setParams] unknown avg contribution [%s]\n", sAvgName);
                                iResult = -1;
                            }
                            ...
                            if (sDevName == "MaternalContribSDev") {
                                ...
                                stdprintf("[OoANavSHybYchMTD2Pop::setParams] unknown sdev contribution [%s]\n", sDevName);
                                iResult = -1;
                            }
  
                        } else {
                            stdprintf("[OoANavSHybYchMTD2Pop::setParams] bad value for pcsdev [%s]\n", vSubs1[1]);
                            iResult = -1;
                        }
                    } else {
                        stdprintf("[OoANavSHybYchMTD2Pop::setParams] expected <ParentContribSDev>=<pcsdev> [%s]\n", sParams);
                        iResult = -1;
                    }
                } else {
                    stdprintf("[OoANavSHybYchMTD2Pop::setParams] bad value for pcavg [%s]\n", vSubs0[1]);
                    iResult = -1;
                }
            ...
            stdprintf("[OoANavSHybYchMTD2Pop::setParams] expected <ParentContribAvg>=<pcavg>+<ParentContribSDev>=<pcsdev> [%s]\n", sParams);
            iResult = -1;

        }
    } else {
        stdprintf("[OoANavSHybYchMTD2Pop::setParams] empty params: do nothing\n");
        iResult = 0;
    }
...
// preLoop
//
int OoANavSHybYchMTD2Pop::preLoop() {
    //    stdprintf("!!!intertest!!! OoANavSHybYchMTD2Pop::preLoop\n");
    // here we should createGenomes (if this is a run started from scratch)
    int iResult = SPopulation<OoANavSHybYchMTD2Agent>::preLoop();
...
//  initializeStep
//
int OoANavSHybYchMTD2Pop::initializeStep(float fCurTime) {
...
//  finalizeStep
//
int OoANavSHybYchMTD2Pop::finalizeStep() {
...
//
int OoANavSHybYchMTD2Pop::updateEvent(int iEventID, char *pData, float fT) {
...
// flushEvents
//
void OoANavSHybYchMTD2Pop::flushEvents(float fT) {
    if (m_bPendingEvents) {
...
//
int OoANavSHybYchMTD2Pop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    int iResult = 0;    
    OoANavSHybYchMTD2Agent &curMother = m_aAgents[iMother];
    OoANavSHybYchMTD2Agent &curFather = m_aAgents[iFather];
    
    OoANavSHybYchMTD2Agent &curAgent = m_aAgents[iAgent];
    curAgent.m_fAge = 0.0;
...
//
int OoANavSHybYchMTD2Pop::addPopSpecificAgentData(int iAgentIndex, char **ppData) {
    int iResult = 0;
    OoANavSHybYchMTD2Agent &newAgent = m_aAgents[iAgentIndex];
    if (iResult == 0) {
    ...
    if (iResult == 0) {
        ...
        if (iResult == 0) {
            ...
            newAgent.m_fParentalHybridization = newAgent.m_fHybridization;
        }
    } else {
...
//
void OoANavSHybYchMTD2Pop::addPopSpecificAgentDataTypeQDF(hid_t *hAgentDataType) {

    OoANavSHybYchMTD2Agent agent;
