...
#include "OoANavGenRelCompPop.h"


//----------------------------------------------------------------------------
// constructor
//
OoANavGenRelCompPop::OoANavGenRelCompPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    : SPopulation<OoANavGenRelCompAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState, aiSeeds),
    ...
    m_pNPPCap = new NPPCapacity<OoANavGenRelCompAgent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride); 

    // prepare parameters for MultiEvaluator, which will compute actual carrying capacity
    MultiEvaluator<OoANavGenRelCompAgent>::evaluatorinfos mEvalInfo;
    ...
    SingleEvaluator<OoANavGenRelCompAgent> *pSEAlt = new SingleEvaluator<OoANavGenRelCompAgent>(this, m_pCG, "Alt", NULL, (double*)m_pCG->m_pGeography->m_adAltitude, "AltCapPref", false, sTriggerIDs);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<OoANavGenRelCompAgent>*>("Multi_weight_alt", pSEAlt));

    // add NPP evaluation - output is NULL because it will be set by MultiEvaluator
    SingleEvaluator<OoANavGenRelCompAgent> *pSECap = new SingleEvaluator<OoANavGenRelCompAgent>(this, m_pCG, "NPP", NULL, m_adCapacities, "", false, sTriggerIDs);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<OoANavGenRelCompAgent>*>("Multi_weight_npp", pSECap));

    m_pME = new MultiEvaluator<OoANavGenRelCompAgent>(this, m_pCG, "Alt+NPP", m_adEnvWeights, mEvalInfo, MODE_MUL_SIMPLE, true); //true: delete evaluators
    addObserver(m_pME);


    m_pBirthDeathRelComp = new BirthDeathRelComp<OoANavGenRelCompAgent>(this, m_pCG, "", m_apWELL, m_adBirthRates, m_aiNumBirths, m_adCapacities, iCapacityStride);

    // evaluator for movement

    m_pWM = new WeightedMove<OoANavGenRelCompAgent>(this, m_pCG, "", m_apWELL, m_adEnvWeights);

    m_pPair = new RandomPair<OoANavGenRelCompAgent>(this, m_pCG, "", m_apWELL);

    m_pGO = new GetOld<OoANavGenRelCompAgent>(this, m_pCG, "");

    m_pOAD = new OldAgeDeath<OoANavGenRelCompAgent>(this, m_pCG, "", m_apWELL);

    m_pFert = new Fertility<OoANavGenRelCompAgent>(this, m_pCG, "");

    //m_pGenetics = new Genetics<OoANavGenRelCompAgent,BitGeneUtils>(this, m_pCG, "", m_pAgentController, &m_vMergedDeadList, m_apWELL);
    m_pGenetics = new Genetics<OoANavGenRelCompAgent,GeneUtils>(this, m_pCG, "", m_pAgentController, &m_vMergedDeadList, m_aiSeeds[1]);

    m_pNavigate = new Navigate<OoANavGenRelCompAgent>(this, m_pCG, "", m_apWELL);

    //    m_pCM = new ConfinedMove<OoANavGenRelCompAgent>(this, m_pCG, "", this->m_vMoveList);
...
// destructor
//
OoANavGenRelCompPop::~OoANavGenRelCompPop() {
...
// setParams
//
int OoANavGenRelCompPop::setParams(const std::string sParams) {
    int iResult = 0;
...
// preLoop
//
int OoANavGenRelCompPop::preLoop() {
    // here we should createGenomes (if this is a run started from scratch)
    int iResult = SPopulation<OoANavGenRelCompAgent>::preLoop();
    ...
    if (iResult == 0) {
        ...
        if (iResult == 0) {
            ...
            printf("[OoANavGenRelCompPop::preLoop] error creating intial genomes\n");
        }
    }
...
//  initializeStep
//
int OoANavGenRelCompPop::initializeStep(float fCurTime) {
...
//  finalizeStep
//
int OoANavGenRelCompPop::finalizeStep() {
...
//
int OoANavGenRelCompPop::updateEvent(int iEventID, char *pData, float fT) {
...
// flushEvents
//
void OoANavGenRelCompPop::flushEvents(float fT) {
    if (m_bPendingEvents) {
...
//
int OoANavGenRelCompPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    int iResult = 0;    

    iResult = m_pGenetics->makeOffspring(iAgent, iMother, iFather);
    OoANavGenRelCompAgent &newAgent = m_aAgents[iAgent];
    newAgent.m_fAge = 0.0;
...
//
int OoANavGenRelCompPop::writeAdditionalDataQDF(hid_t hSpeciesGroup) {
...
//
int OoANavGenRelCompPop::addPopSpecificAgentData(int iAgentIndex, char **ppData) {
    int iResult = 0;
    if (iResult == 0) {
...
//
void OoANavGenRelCompPop::addPopSpecificAgentDataTypeQDF(hid_t *hAgentDataType) {

    OoANavGenRelCompAgent agent;
...
//
int OoANavGenRelCompPop::readAdditionalDataQDF(hid_t hSpeciesGroup) {
...
//
int  OoANavGenRelCompPop::dumpAdditionalDataQDF(hid_t hSpeciesGroup) {

    int iResult = m_pGenetics->dumpAdditionalDataQDF(hSpeciesGroup);
    printf("[OoANavGenRelCompPop::dumpAdditionalDataQDF] gendump returned %d\n", iResult);
    return iResult;
}
...
//
int  OoANavGenRelCompPop::restoreAdditionalDataQDF(hid_t hSpeciesGroup) {
...
//
int OoANavGenRelCompPop::copyAdditionalDataQDF(int iStart, int iCount, SPopulation *pPop) {
    OoANavGenRelCompPop *pO = static_cast<OoANavGenRelCompPop*>(pPop);
