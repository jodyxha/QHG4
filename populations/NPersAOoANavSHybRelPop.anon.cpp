...
// constructor
//
NPersXOoANavSHybRelPop::NPersXOoANavSHybRelPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    : SPopulation<NPersXOoANavSHybRelAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState, aiSeeds),
...
int NPersXOoANavSHybRelPop::setParams(const std::string sParams) {
    ...
    if (pParams != NULL) {
        if (strToNum(pParams, &m_dSDev)) {
            iResult = 0;
        } else {
            iResult = -1;
            printf("[NPersXOoANavSHybRelPop::setParams] expected a float, not [%s]\n", pParams);
        }
    } 
    return iResult;
}
...
int NPersXOoANavSHybRelPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    ...
    */
    ...
    //printf("makePopSpecificOffspring A%d before gauss1 (sig%f, prev %f)\n", iAgent, curMother.m_fNeaSDev, m_apWELL[0]->getPrevNormal());
    //WELLUtils::showStates(m_apWELL, omp_get_max_threads(), false);

    //standard deviation for that n: sdev(n, sdev0) = sdev0 * (1-2*abs(0.5-n)).
    double nM = curMother.m_fNeaFrac + this->m_apWELL[0]->wgauss(curMother.m_fNeaSDev);
    //printf("makePopSpecificOffspring A%d before gauss2 (sig%f, prev %f)\n", iAgent, curFather.m_fNeaSDev, m_apWELL[0]->getPrevNormal());
    //WELLUtils::showStates(m_apWELL, omp_get_max_threads(), false);
    double nP = curFather.m_fNeaFrac + this->m_apWELL[0]->wgauss(curFather.m_fNeaSDev);
...
int NPersXOoANavSHybRelPop::addPopSpecificAgentData(int iAgentIndex, char **ppData) {
    ...
    if (iResult == 0) {
        ...
        printf("[addPopSpecificAgentData] Couldn't read m_fNeaFrac from [%s]\n", *ppData);
    }
    ...
    if (iResult == 0) {
        ...
        printf("[addPopSpecificAgentData] Couldn't read m_fNeaSDev from [%s]\n", *ppData);
    }
