...
typedef PopBase           *(*createPopFunc)(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds);
...
int DynPopFactory::collectPlugins() {
    ...
    for (uint i = 0; (iResult == 0) && (i < m_vSODirs.size()); i++) {
        iResult = collectPlugins(m_vSODirs[i]);
    }
...
// collectPlugins
//
int DynPopFactory::collectPlugins(const std::string sPath) {
    int iResult = -1;
    ...
    if (it != m_mNameFiles.end()) {
        ...
        if (hLibrary != NULL) {
            ...
            if (pCreatePop != NULL) {
                ...
                pPB = pCreatePop(m_pCG, m_pPopFinder, m_iLayerSize, m_apIDG, m_aulState, m_aiSeeds);
            } else {
