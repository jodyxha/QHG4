...
#include "OoANavGenPhenRelPop.h"


//----------------------------------------------------------------------------
// constructor
//
OoANavGenPhenRelPop::OoANavGenPhenRelPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    : SPopulation<OoANavGenPhenRelAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState, aiSeeds),
    ...
    m_pNPPCap = new NPPCapacity<OoANavGenPhenRelAgent>(this, m_pCG, "", m_apWELL, m_adCapacities, iCapacityStride); 

    // prepare parameters for MultiEvaluator, which will compute actual carrying capacity
    MultiEvaluator<OoANavGenPhenRelAgent>::evaluatorinfos mEvalInfo;
 
    // add altitude evaluation - output is NULL because it will be set by MultiEvaluator
    SingleEvaluator<OoANavGenPhenRelAgent> *pSEAlt = new SingleEvaluator<OoANavGenPhenRelAgent>(this, m_pCG, "Alt", NULL, (double*)m_pCG->m_pGeography->m_adAltitude, "AltCapPref", true, EVENT_ID_GEO);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<OoANavGenPhenRelAgent>*>("Multi_weight_alt", pSEAlt));

    // add NPP evaluation - output is NULL because it will be set by MultiEvaluator
    SingleEvaluator<OoANavGenPhenRelAgent> *pSECap = new SingleEvaluator<OoANavGenPhenRelAgent>(this, m_pCG, "NPP", NULL, m_adCapacities, "NPPPref", true, EVENT_ID_VEG);
    mEvalInfo.push_back(std::pair<std::string, Evaluator<OoANavGenPhenRelAgent>*>("Multi_weight_npp", pSECap));

    m_pME = new MultiEvaluator<OoANavGenPhenRelAgent>(this, m_pCG, "Alt+NPP", m_adEnvWeights, mEvalInfo, MODE_ADD_SIMPLE, true); //true: delete evaluators
    addObserver(m_pME);


    m_pBirthDeathRel = new BirthDeathRel<OoANavGenPhenRelAgent>(this, m_pCG, "", m_apWELL, m_adBirthRates, m_aiNumBirths, m_adCapacities, iCapacityStride);


    // evaluator for movement

    m_pWM = new WeightedMove<OoANavGenPhenRelAgent>(this, m_pCG, "", m_apWELL, m_adEnvWeights);

    m_pPair = new RandomPair<OoANavGenPhenRelAgent>(this, m_pCG, "", m_apWELL);

    m_pGO = new GetOld<OoANavGenPhenRelAgent>(this, m_pCG, "");

    m_pOAD = new OldAgeDeath<OoANavGenPhenRelAgent>(this, m_pCG, "", m_apWELL);

    m_pFert = new Fertility<OoANavGenPhenRelAgent>(this, m_pCG, "");

    //m_pGenetics = new Genetics<OoANavGenPhenRelAgent,BitGeneUtils>(this, m_pCG, "", m_pAgentController, &m_vMergedDeadList, m_apWELL);
    m_pGenetics = new Genetics<OoANavGenPhenRelAgent,BitGeneUtils>(this, m_pCG, "", m_pAgentController, &m_vMergedDeadList, m_aiSeeds[1]);

    //m_pPhenetics = new Phenetics<OoANavGenPhenRelAgent>(this, m_pCG, "", m_pAgentController, &m_vMergedDeadList, m_apWELL);
    m_pPhenetics = new Phenetics2<OoANavGenPhenRelAgent>(this, m_pCG, "", m_pAgentController, &m_vMergedDeadList, m_aiSeeds[2]);

    m_pNavigate = new Navigate<OoANavGenPhenRelAgent>(this, m_pCG, "", m_apWELL);

    //    m_pCM = new ConfinedMove<OoANavGenPhenRelAgent>(this, m_pCG, "", this->m_vMoveList);
...
// destructor
//
OoANavGenPhenRelPop::~OoANavGenPhenRelPop() {
...
// setParams
//
int OoANavGenPhenRelPop::setParams(const std::string sParams) {
    int iResult = 0;
...
// preLoop
//
int OoANavGenPhenRelPop::preLoop() {
    // here we should createGenomes (if this is a run started from scratch)
    int iResult = SPopulation<OoANavGenPhenRelAgent>::preLoop();
    ...
    if (iResult == 0) {
        ...
        if (iResult == 0) {
            ...
            if (iResult == 0) {
                ...
                printf("[OoANavGenPhenRelPop::preLoop] error creating intial phenomes\n");
            }
        } else {
            printf("[OoANavGenPhenRelPop::preLoop] error creating intial genomes\n");
        }
    }
...
//  initializeStep
//
int OoANavGenPhenRelPop::initializeStep(float fCurTime) {
...
//  finalizeStep
//
int OoANavGenPhenRelPop::finalizeStep() {
...
//
int OoANavGenPhenRelPop::updateEvent(int iEventID, char *pData, float fT) {
...
// flushEvents
//
void OoANavGenPhenRelPop::flushEvents(float fT) {
    if (m_bPendingEvents) {
...
//
int OoANavGenPhenRelPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    int iResult = 0;    

    iResult = m_pGenetics->makeOffspring(iAgent, iMother, iFather);

    iResult = m_pPhenetics->makeOffspring(iAgent, iMother, iFather);
    OoANavGenPhenRelAgent &newAgent =  m_aAgents[iAgent];
    newAgent.m_fAge = 0.0;
...
//
int OoANavGenPhenRelPop::writeAdditionalDataQDF(hid_t hSpeciesGroup) {
...
//
int OoANavGenPhenRelPop::addPopSpecificAgentData(int iAgentIndex, char **ppData) {
    int iResult = 0;
    if (iResult == 0) {
...
//
void OoANavGenPhenRelPop::addPopSpecificAgentDataTypeQDF(hid_t *hAgentDataType) {

    OoANavGenPhenRelAgent agent;
...
//
int OoANavGenPhenRelPop::readAdditionalDataQDF(hid_t hSpeciesGroup) {
...
//
int  OoANavGenPhenRelPop::dumpAdditionalDataQDF(hid_t hSpeciesGroup) {

    int iResult = m_pGenetics->dumpAdditionalDataQDF(hSpeciesGroup);
    printf("[OoANavGenPhenRelPop::dumpAdditionalDataQDF] gendump returned %d\n", iResult);
    if (iResult == 0) {
        iResult = m_pPhenetics->dumpAdditionalDataQDF(hSpeciesGroup);
        printf("[OoANavGenPhenRelPop::dumpAdditionalDataQDF] phendump returned %d\n", iResult);
    }
    return iResult;
}
...
//
int  OoANavGenPhenRelPop::restoreAdditionalDataQDF(hid_t hSpeciesGroup) {
...
//
int OoANavGenPhenRelPop::copyAdditionalDataQDF(int iStart, int iCount, SPopulation *pPop) {
    OoANavGenPhenRelPop *pO = static_cast<OoANavGenPhenRelPop *>(pPop);
