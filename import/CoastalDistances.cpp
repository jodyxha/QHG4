...
#pragma omp parallel for
    for (uint j = 0; j < m_mCoastRanges.size(); ++j) {
     ...
     for (it = m_vCoastCells.begin(); (iResult == 0) && (it != m_vCoastCells.end()); ++it) {
        ...
        if (sCoast.size() > 2) {
            ...
            for (it2 = sCoast.begin(); it2 != sCoast.end(); ++it2) {
                ...
                std::vector<int>::iterator it3 = std::set_intersection(sWater.begin(), sWater.end(), sOtherWater.begin(), sOtherWater.end(), v.begin());
                /*
                v.resize(it-v.begin());
                if (v.size() > 0) {
                */
