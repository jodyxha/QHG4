...
//
template<typename T>
int ChildManager<T>::extractParamsQDF(hid_t hSpeciesGroup) {
    int iResult = 0;
...
//
template<typename T>
int ChildManager<T>::writeParamsQDF(hid_t hSpeciesGroup) {
    int iResult = 0;
...
int ChildManager<T>::tryGetParams(const ModuleComplex *pMC) {
    ...
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_CHILDMAN_ADULT_AGE_NAME,    &m_fAdultAge);
    }
    return iResult;
}
