...
// setName
//
void qhgNode::setName(const std::string sName) {
    m_sName = sName;
}
...
//
qhgXML *qhgXML::createInstance(const std::string sFile) {
...
//   artficial "root" node
//
int qhgXML::init(const std::string sFile) {
    int iResult = -1;

    LineReader *pLR = LineReader_std::createInstance(sFile, "r");
    if (pLR != NULL) {
        // build the root node
