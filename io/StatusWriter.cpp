...
int StatusWriter::write(const std::string sFileName, int iStep, float fStartTime, const std::string sInfoString, int iWhat, int iDumpMode) {
    std::vector<std::pair<std::string, int>> vEmptySub;
...
//   write elements specified by iWhat=ored combination of WR_XXX constants
//
int StatusWriter::write(const std::string sFileName, int iStep, float fStartTime, const std::string sInfoString, int iWhat, std::vector<std::pair<std::string, int>> &vSub, int iDumpMode) {
    int iResult = iWhat;
    int iTemp = 0;

    m_hFile = qdf_createFile(sFileName, iStep, fStartTime, sInfoString);
    if (m_hFile > 0) {
	// there may be several populations - each contributing WR_POP
        if (((iResult / WR_POP) > 0) && (m_pPopW != NULL)) {
