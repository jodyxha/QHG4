...
// addParam
//
int ModuleComplex::addParam(const std::string sKey, const char *pVal) {
    int iResult = -1;
...
//----------------------------------------------------------------------------
// getAttribute
//  reeturn NULL if name does not exist
//
...
// createInstance
//
ParamProvider2 *ParamProvider2::createInstance(const std::string sXML) {
    ParamProvider2 *pQXR = new ParamProvider2();
    int iResult = pQXR->init(sXML);
    if (iResult != 0)  {
        delete pQXR;
        pQXR = NULL;
    }
...
//
int ParamProvider2::processParam(qhgNode *pParam, const std::string sSubtag,  stringmap &att_param) {
    int iResult = -1;
...
// processModule
//
int ParamProvider2::processModule(qhgNode *pModule, modulemap &mModules) {
    int iResult = -1;
    if (pModule != NULL) {
        if (pModule->getName() == ELEM_MODULE) {
            ...
            if (!sName.empty()) {
                ...
                qhgNode *pChild = pModule->getChild();
...
// processPriorities
//
int ParamProvider2::processPriorities(qhgNode *pPrios, stringmap &pa) {
    int iResult = -1;
    if (pPrios != NULL) {
        if (pPrios->getName() == ELEM_PRIOS) {
            iResult = 0;
            //stdprintf("[ParamProvider2::processPriorities] processing priorities\n");
            qhgNode *pChild = pPrios->getChild();
...
// processClass
//
int ParamProvider2::processClass(qhgNode *pClass) {
    int iResult = -1;
    if (pClass != NULL) {
        if (pClass->getName() == ELEM_CLASS) {
            ...
            if (!sName.empty()) {
                ...
                if (it != attr_class.end()) {
                    ...
                    qhgNode *pChild = pClass->getChild();
                    while ((iResult == 0) && (pChild != NULL)) {
...
// init
//
int ParamProvider2::init(const std::string sXML) {
    int iResult = -1;
    qhgXML *pQX = qhgXML::createInstance(sXML);
    if (pQX != NULL) {
        qhgNode  *pRoot = pQX->getRoot(); 

        qhgNode *pClass = pRoot->getChild();
        iResult = 0;
...
void showModule(ModuleComplex *pModule, const std::string sIndent) {
    ...
    for (itp = pModule->getParams().begin(); itp != pModule->getParams().end(); ++itp) {
