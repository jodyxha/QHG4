...
//  closes file
//
int PopWriter::write(const std::string sFilename, int iStep, float fStartTime, const std::string sInfoString, const std::string sSub, int iWSpecial, int iDumpMode) {
    int iResult = -1;
...
//
int PopWriter::write(hid_t hFile, const std::string sSub, int iWSpecial, int iDumpMode) {
    int iResult = 0;
        ...
        if (m_hFile > 0) {
            ...
            if (iResult == 0) {
                ...
                if (iResult == 0) {
                    ...
                    if (iDumpMode != -1) {
                        pPop->dumpStatArrsQDF(m_hSpeciesGroup);
                        pPop->dumpAdditionalDataQDF(m_hSpeciesGroup);
                    } else {
                        // write stat & cap arrays  if required
                        if ((iWSpecial & PW_STATS_ONLY) != 0) {
                            pPop->writeStatArrsQDF(m_hSpeciesGroup);
                        }
                        // write additional data  if required
                        if ((iWSpecial & PW_ADDITIONAL_ONLY) != 0) {
