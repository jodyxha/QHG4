...
//
template<typename T>
int Phenetics2<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int Phenetics2<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int Phenetics2<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_PHENETICS_PHENOME_SIZE,       &m_iPhenomeSize);      
        iResult += getParamVal(mParams, ATTR_PHENETICS_MUTATION_RATE,      &m_dMutationRate);     
        iResult += getParamVal(mParams, ATTR_PHENETICS_MUTATION_SIGMA,     &m_dMutationSigma);    
        iResult += getParamVal(mParams, ATTR_PHENETICS_INITIAL_SIGMA,      &m_dInitialSigma);     
        iResult += getParamVal(mParams, ATTR_PHENETICS_CREATE_NEW_PHENOME, &m_bCreateNewPhenome); 
        iResult += getParamVal(mParams, ATTR_PHENETICS_MIX_AVG,            &m_bMixAvg);           
        iResult += getParamVal(mParams, ATTR_PHENETICS_MUTATION_TYPE,      &m_iMutType);          
        iResult += getParamVal(mParams, ATTR_PHENETICS_PLOIDY,             &m_iPloidy);           
    }
    return iResult;
}
