...
//
template<typename T>
int NPersHybLinearBirthRel<T>::operator()(int iAgentIndex, float fT) {
...
#pragma omp critical
                {
                    printf("agent %d: b0 %f, theta %f, numags %lu, cc %f -> bReal %f\n", iAgentIndex, pa->m_dB0, pa->m_dTheta, this->m_pPop->getNumAgents(iCellIndex), pa->m_dCC,  pa->m_dBReal);
                } 
            }               
            */
...
//
template<typename T>
int NPersHybLinearBirthRel<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int NPersHybLinearBirthRel<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int NPersHybLinearBirthRel<T>::modifyParams(const std::string sAttrName, double dValue) {
    int iResult = 0;
...
//
template<typename T>
int NPersHybLinearBirthRel<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_NPERSHYBLINBIRTHREL_HYBMINPROB_NAME, &m_dHybMinProb);
    }
    return iResult;
}
