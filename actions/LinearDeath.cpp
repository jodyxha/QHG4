...
//
template<typename T>
int LinearDeath<T>::modifyParams(const std::string sAttrName, double dValue) {
    int iResult = 0;
...
//
template<typename T>
int LinearDeath<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {  
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_LINDEATH_D0_NAME, &m_dD0);           
        iResult += getParamVal(mParams, ATTR_LINDEATH_TURNOVER_NAME, &m_dTheta);  
        iResult += getParamVal(mParams, ATTR_LINDEATH_CAPACITY_NAME, &m_dK);      
    }
    return iResult;
}
