...
//
template<typename T>
int NPersAHybBirthDeathRel<T>::operator()(int iAgentIndex, float fT) {
...
//
template<typename T>
int NPersAHybBirthDeathRel<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int NPersAHybBirthDeathRel<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int NPersAHybBirthDeathRel<T>::modifyParams(const std::string sAttrName, double dValue) {
...
//
template<typename T>
int NPersAHybBirthDeathRel<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_NPERSAHYBBIRTHDEATHREL_HYBMINPROB_NAME, &m_dHybMinProb);
        if (iResult == 0) {
            // just in case we're reading in new parameters,
