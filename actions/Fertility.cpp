...
//
template<typename T>
int Fertility<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_FERTILITY_MIN_AGE_NAME, &m_fFertilityMinAge);
        iResult += getParamVal(mParams, ATTR_FERTILITY_MAX_AGE_NAME, &m_fFertilityMaxAge);
        iResult += getParamVal(mParams, ATTR_FERTILITY_INTERBIRTH_NAME, &m_fInterbirth);  
    }
    return iResult;
}
