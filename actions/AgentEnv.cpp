...
//
template<typename T>
int AgentEnv<T>::extractParamsQDF(hid_t hSpeciesGroup) {
    ...
    if (iResult == 0) {
        ...
        if (iResult != 0) {
            LOG_ERROR("[FoodManager<T>::extractParamsQDF] couldn't read attribute [%s]", ATTR_AGENTENV_REQUIREMENTS_NAME);
        }
    }
    return iResult;
}
...
//
template<typename T>
int AgentEnv<T>::writeParamsQDF(hid_t hSpeciesGroup) {
    int iResult = 0;
...
//
template<typename T>
int AgentEnv<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;
 
    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {  
        iResult += getParamVal(mParams, ATTR_AGENTENV_REQUIREMENTS_NAME, &m_dRequirements);         
    }
    return iResult;
}
