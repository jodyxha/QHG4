...
template<typename T>
const stringvec LocEnv2<T>::s_vNames = {
    ATTR_LOCENV2_WATERFACTOR_NAME + "_sapiens",
    ATTR_LOCENV2_COASTALFACTOR_NAME +  "_sapiens",
    ATTR_LOCENV2_COASTAL_MIN_LAT_NAME + "_sapiens",
    ATTR_LOCENV2_COASTAL_MAX_LAT_NAME + "_sapiens",
    ATTR_LOCENV2_NPPMIN_NAME + "_sapiens",
    ATTR_LOCENV2_NPPMAX_NAME + "_sapiens",
    ATTR_LOCENV2_KMAX_NAME + "_sapiens",
    ATTR_LOCENV2_KMIN_NAME + "_sapiens",
    ATTR_LOCENV2_ALT_PREF_POLY_NAME + "_sapiens",

    ATTR_LOCENV2_WATERFACTOR_NAME + "_neander",
    ATTR_LOCENV2_COASTALFACTOR_NAME + "_neander",
    ATTR_LOCENV2_COASTAL_MIN_LAT_NAME + "_neander",
    ATTR_LOCENV2_COASTAL_MAX_LAT_NAME + "_neander",
    ATTR_LOCENV2_NPPMIN_NAME + "_neander",
    ATTR_LOCENV2_NPPMAX_NAME + "_neander",
    ATTR_LOCENV2_KMAX_NAME + "_neander",
    ATTR_LOCENV2_KMIN_NAME + "_neander",
    ATTR_LOCENV2_ALT_PREF_POLY_NAME + "_neander",
};

template<typename T>
const std::string LocEnv2<T>::asSpecies[] = {"sapiens", "neander"};
...
//
template<typename T>
LocEnv2<T>::LocEnv2(SPopulation<T> *pPop,  SCellGrid *pCG, std::string sID, WELL512** apWELL, LBController *pAgentController) 
    : Action<T>(pPop, pCG, ATTR_LOCENV2_NAME,sID),
...
//
template<typename T>
void LocEnv2<T>::notify(Observable *pObs, int iEvent, const void *pData) {
...
//
template<typename T>
const double *LocEnv2<T>::getArr(int iAgentIndex) {
...
int LocEnv2<T>::init() {
    ...
    if ((this->m_pCG != NULL) &&
        ...
        if (this->m_pCG == NULL) {
            stdprintf("[LocEnv2] No cell grid!\n");
        } else {
            if (this->m_pCG->m_pGeography  == NULL) {
                stdprintf("[LocEnv2] Cellgrid has no geography\n");
            }
            if (this->m_pCG->m_pClimate    == NULL) {
                stdprintf("[LocEnv2] Cellgrid has no climate\n");
            }
            if (this->m_pCG->m_pVegetation == NULL) {
                stdprintf("[LocEnv2] Cellgrid has no vegetation\n");
            }
        }
    }
    return iResult;
...
//
template<typename T>
int LocEnv2<T>::recalculateGlobalCapacities() {
    if (m_bNeedUpdate) {
        stdprintf("LocEnv2::recalculateGlobalCapacities\n");
        int iNumCells = this->m_pCG->m_iNumCells; 
...
//
template<typename T>
int LocEnv2<T>::initialize(float fTime) { 
    int iResult = 0;
...
#pragma omp parallel for
    for (int iA = iFirstAgent; iA <= iLastAgent; iA++) {
        ...
        float fH     = pA.m_fParentalHybridization;  // this basically the only difference to LocEnv
...
//      ATTR_LOCENV2_WATERFACTOR_NAME
//      ATTR_LOCENV2_COASTALFACTOR_NAME
//      ATTR_LOCENV2_COASTAL_MIN_LAT_NAME
//      ATTR_LOCENV2_COASTAL_MAX_LAT_NAME
//      ATTR_LOCENV2_NPPMIN_NAME
//      ATTR_LOCENV2_NPPMAX_NAME
//      ATTR_LOCENV2_KMAX_NAME
//      ATTR_LOCENV2_KMIN_NAME
//      ATTR_LOCENV2_EFFICIENCY_NAME
//   each in 2 
template<typename T>
int LocEnv2<T>::extractParamsQDF(hid_t hSpeciesGroup) {
    ...
    for (int iGroup = 0; (iResult == 0) && (iGroup < 2); iGroup++) {
        if (iResult == 0) {
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_WATERFACTOR_NAME, asSpecies[iGroup]);
            iResult = qdf_extractAttribute(hSpeciesGroup, sNameID, 1, &(m_dWaterFactor[iGroup]));
            if (iResult != 0) {
                LOG_ERROR("[LocEnv2] couldn't read attribute [%s]", sNameID);
            }
        }

        if (iResult == 0) {
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_COASTALFACTOR_NAME, asSpecies[iGroup]);
        ...
        if (iResult == 0) {
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_COASTAL_MIN_LAT_NAME, asSpecies[iGroup]);
        ...
        if (iResult == 0) {
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_COASTAL_MAX_LAT_NAME, asSpecies[iGroup]);
        ...
        if (iResult == 0) {
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_NPPMIN_NAME, asSpecies[iGroup]);
        ...
        if (iResult == 0) {
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_NPPMAX_NAME, asSpecies[iGroup]);
        ...
        if (iResult == 0) {
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_KMAX_NAME, asSpecies[iGroup]);
        ...
        if (iResult == 0) {
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_KMIN_NAME, asSpecies[iGroup]);
        ...
        if (iResult == 0) {
             ...
             m_sPLParName[iGroup] = stdsprintf(sMask, ATTR_LOCENV2_ALT_PREF_POLY_NAME, asSpecies[iGroup]);
             stdprintf("SingleEvaluator::extractParamsQDF will work on %s\n", m_sPLParName[iGroup]);
...
//      ATTR_LOCENV2_VEGSELECTION_NAME
//      ATTR_LOCENV2_WATERFACTOR_NAME
//      ATTR_LOCENV2_COASTALFACTOR_NAME
//      ATTR_LOCENV2_COASTAL_MIN_LAT_NAME
//      ATTR_LOCENV2_COASTAL_MAX_LAT_NAME
...
//
template<typename T>
int LocEnv2<T>::writeParamsQDF(hid_t hSpeciesGroup) {
    ...
    //    iResult += qdf_insertAttribute(hSpeciesGroup, ATTR_LOCENV2_VEGSELECTION_NAME, 3, m_adSelection);
    for (int iGroup = 0; (iResult == 0) && (iGroup < 2); iGroup++) {
        sNameID = stdsprintf(sMask, ATTR_LOCENV2_WATERFACTOR_NAME, asSpecies[iGroup]);
...
//
template<typename T>
int LocEnv2<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;
    std::string sNameID;
    const char *sMask = "%s_%s";
    ...
    if (this->checkParams(mParams) == 0) {
        ...
        for (int iGroup = 0; (iResult == 0) && (iGroup < 2); iGroup++) {
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_WATERFACTOR_NAME, asSpecies[iGroup]);
            iResult += getParamVal(mParams, sNameID, &(m_dWaterFactor[iGroup]));            
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_COASTALFACTOR_NAME, asSpecies[iGroup]);
            iResult += getParamVal(mParams, sNameID, &(m_dCoastalFactor[iGroup]));        
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_COASTAL_MIN_LAT_NAME, asSpecies[iGroup]);
            iResult += getParamVal(mParams, sNameID, &(m_dCoastalMinLatitude[iGroup])); 
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_COASTAL_MAX_LAT_NAME, asSpecies[iGroup]);
            iResult += getParamVal(mParams, sNameID, &(m_dCoastalMaxLatitude[iGroup])); 
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_NPPMIN_NAME, asSpecies[iGroup]);
            iResult += getParamVal(mParams, sNameID, &(m_dNPPMin[iGroup]));                      
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_NPPMAX_NAME, asSpecies[iGroup]);
            iResult += getParamVal(mParams, sNameID, &(m_dNPPMax[iGroup]));                      
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_KMAX_NAME, asSpecies[iGroup]);
            iResult += getParamVal(mParams, sNameID, &(m_dKMax[iGroup]));                          
            sNameID = stdsprintf(sMask, ATTR_LOCENV2_KMIN_NAME, asSpecies[iGroup]);
            iResult += getParamVal(mParams, sNameID, &(m_dKMin[iGroup]));        

            std::string sPolyDesc = "";
            m_sPLParName[iGroup] = stdsprintf(sMask, ATTR_LOCENV2_ALT_PREF_POLY_NAME, asSpecies[iGroup]);
            iResult = getParamStr(mParams, m_sPLParName[iGroup],  sPolyDesc);
            if (iResult == 0) {
...
//
template<typename T>
bool LocEnv2<T>::isEqual(Action<T> *pAction, bool bStrict) {
    bool bEqual = true;
    LocEnv2<T>* pA = static_cast<LocEnv2<T>*>(pAction);
    for (int iGroup = 0; iGroup < 2; iGroup++) {
