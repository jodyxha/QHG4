...
//
template<typename T>
int CondWeightedMove<T>::modifyParams(const std::string sAttrName, double dValue) {
...
//
template<typename T>
int CondWeightedMove<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = getParamVal(mParams,  ATTR_CONDWEIGHTEDMOVE_PROB_NAME, &m_dMoveProb);
    }
    return iResult;
}
