...
//
template<typename T>
int LotkaVolterra<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;       
        iResult += this->getParamVal(mParams, ATTR_LOTKAVOLTERRA_SELFRATE_NAME, 1, &m_dSelfRate);
        iResult += this->getParamVal(mParams, ATTR_LOTKAVOLTERRA_MIXRATE_NAME, 1, &m_dMixRate);  
        iResult += this->getParamVal(mParams, ATTR_LOTKAVOLTERRA_OTHERPOP_NAME, m_sOtherPopname);
        iResult += this->getParamVal(mParams, ATTR_LOTKAVOLTERRA_K_NAME, 1, &m_dK);              
    }
    return iResult;
}
