...
//
template<typename T>
int HybBirthDeathRel<T>::operator()(int iAgentIndex, float fT) {
...
//
template<typename T>
int HybBirthDeathRel<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int HybBirthDeathRel<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int HybBirthDeathRel<T>::modifyParams(const std::string sAttrName, double dValue) {
...
//
template<typename T>
int HybBirthDeathRel<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_HYBBIRTHDEATHREL_B0_NAME, &m_dB0);                
        iResult += getParamVal(mParams, ATTR_HYBBIRTHDEATHREL_D0_NAME, &m_dD0);                
        iResult += getParamVal(mParams, ATTR_HYBBIRTHDEATHREL_THETA_NAME, &m_dTheta);          
        iResult += getParamVal(mParams, ATTR_HYBBIRTHDEATHREL_HYBMINPROB_NAME, &m_dHybMinProb);
        if (iResult == 0) {
            // just in case we're reading in new parameters,
