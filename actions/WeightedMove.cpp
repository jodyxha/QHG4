...
//
template<typename T>
int WeightedMove<T>::modifyParams(const std::string sAttrName, double dValue) {
...
//
template<typename T>
int WeightedMove<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;
    

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = getParamVal(mParams, ATTR_WEIGHTEDMOVE_PROB_NAME, &m_dMoveProb);
    }
    return iResult;
}
