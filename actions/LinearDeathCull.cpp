...
//
template<typename T>
int LinearDeathCull<T>::operator()(int iAgentIndex, float fT) {
...
//
template<typename T>
int LinearDeathCull<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int LinearDeathCull<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int LinearDeathCull<T>::modifyParams(const std::string sAttrName, double dValue) {
    int iResult = 0;
...
//
template<typename T>
int LinearDeathCull<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_LINDEATHCULL_D0_NAME, &m_dD0);         
        iResult += getParamVal(mParams, ATTR_LINDEATHCULL_TURNOVER_NAME, &m_dTheta);
        iResult += getParamVal(mParams, ATTR_LINDEATHCULL_CAPACITY_NAME, &m_dK);    
        iResult += getParamVal(mParams, ATTR_LINDEATHCULL_EPS_NAME, &m_dEps);       
    }
    return iResult;
}
