...
//
template<typename T>
int HybLinearBirthRel<T>::operator()(int iAgentIndex, float fT) {
...
//
template<typename T>
int HybLinearBirthRel<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int HybLinearBirthRel<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int HybLinearBirthRel<T>::modifyParams(const std::string sAttrName, double dValue) {
    int iResult = 0;
...
//
template<typename T>
int HybLinearBirthRel<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_HYBLINBIRTHREL_B0_NAME, &m_dB0);           
        iResult += getParamVal(mParams, ATTR_HYBLINBIRTHREL_TURNOVER_NAME, &m_dTheta);  
        iResult += getParamVal(mParams, ATTR_HYBLINBIRTHREL_HYBMINPROB_NAME, &m_dHybMinProb);
    }
    return iResult;
}
