...
int PDHunting<T>::operator()(int iAgentIndex, float fT) {
    ...
    if (pAss != NULL) {
        ...
        /*debug:
        stdprintf("[PDHunting<T>::operator()] no assmap for [%s]\n", this->m_pPop->getSpeciesName());
        */
    }
...
//
template<typename T>
int PDHunting<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = getParamStr(mParams,  ATTR_PDHUNTING_RELATIONS_NAME, m_sRelationInput);  
    }
    return iResult;
}
