...
//
template<typename T> 
int FoodManager<T>::extractParamsQDF(hid_t hSpeciesGroup) {
    int iResult = 0;

    if (iResult == 0) {
        iResult = qdf_extractAttribute(hSpeciesGroup, ATTR_FOODMAN_GROWTH_RATE_NAME, 1, &m_dFoodGrowthRate);
        if (iResult != 0) {
            LOG_ERROR("[FoodManager<T>::extractParamsQDF] couldn't read attribute [%s]", ATTR_FOODMAN_GROWTH_RATE_NAME);
        }
    }
    if (iResult == 0) {
        iResult = qdf_extractAttribute(hSpeciesGroup, ATTR_FOODMAN_MIN_AMOUNT_NAME, 1, &m_dFoodMinAmount);
        if (iResult != 0) {
            LOG_ERROR("[FoodManager<T>::extractParamsQDF] couldn't read attribute [%s]", ATTR_FOODMAN_MIN_AMOUNT_NAME);
        }
    }
    return iResult;
}
...
//
template<typename T> 
int FoodManager<T>::writeParamsQDF(hid_t hSpeciesGroup) {
    int iResult = 0;
...
//
template<typename T> 
int FoodManager<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {  
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_FOODMAN_GROWTH_RATE_NAME, &m_dFoodGrowthRate);         
        iResult += getParamVal(mParams, ATTR_FOODMAN_MIN_AMOUNT_NAME,  &m_dFoodMinAmount);
