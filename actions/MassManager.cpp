...
//
template<typename T>
int MassManager<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {  
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_MASSMANAGER_MIN_NAME,   &m_dMinMass); 
        iResult += getParamVal(mParams, ATTR_MASSMANAGER_MAX_NAME,   &m_dMaxMass); 
        iResult += getParamVal(mParams, ATTR_MASSMANAGER_DELTA_NAME, &m_dDelta);   
    }
    return iResult;
}
