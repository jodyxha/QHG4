...
//
template<typename T, class U>
int RandDistrPair<T,U>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams,  ATTR_RANDDISTRPAIR_DCRIT,        &m_fDCrit);
        iResult += getParamVal(mParams,  ATTR_RANDDISTRPAIR_VCRIT,        &m_fVCrit);
    }
    return iResult;
}
