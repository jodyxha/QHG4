...
//
template<typename T>
int Birther<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_BIRTHER_ADULTMASS_NAME,   &m_dAdultMass);   
        iResult += getParamVal(mParams, ATTR_BIRTHER_BIRTHMASS_NAME,   &m_dBirthMass);   
        iResult += getParamVal(mParams, ATTR_BIRTHER_UNCERTAINTY_NAME, &m_dUncertainty); 
    }
    return iResult;
}
