...
//
template<typename T>
int PrivParamMix<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int PrivParamMix<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int PrivParamMix<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;
    std::string sNameID;
    const char *sMask = "%s_%s";
    ...
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_PRIVPARAMMIX_MODE_NAME, &m_iMode);            
