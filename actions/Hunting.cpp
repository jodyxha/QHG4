...
//
template<typename T>
int Hunting<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {  
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_HUNTING_EFFICIENCY_NAME, &m_dEfficiency);  
        iResult += getParamVal(mParams, ATTR_HUNTING_USABILITY_NAME,  &m_dUsability);   
        iResult += getParamStr(mParams, ATTR_HUNTING_PREYSPECIES_NAME, m_sPreyPopName);
    }
    return iResult;
}
