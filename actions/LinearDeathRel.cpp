...
//
template<typename T>
int LinearDeathRel<T>::operator()(int iAgentIndex, float fT) {
...
//
template<typename T>
int LinearDeathRel<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int LinearDeathRel<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int LinearDeathRel<T>::modifyParams(const std::string sAttrName, double dValue) {
    int iResult = 0;
...
//
template<typename T>
int LinearDeathRel<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_LINDEATHREL_D0_NAME, &m_dD0);         
        iResult += getParamVal(mParams, ATTR_LINDEATHREL_TURNOVER_NAME, &m_dTheta);
    }
    return iResult;
}
