...
int MultiEvaluator<T>::extractParamsQDF(hid_t hSpeciesGroup) {
    ...
    for (int i = 0; i < m_iNumEvals && iResult == 0; i++) {
        ...
        if (iResult == 0) {
            if (this->m_pPop->getQDFVersionIn() >= 4) {
                hid_t hActionGroup = qdf_openGroup(hSpeciesGroup, m_aEvaluators[i]->getActionName());
                if (hActionGroup != H5P_DEFAULT) {
                    iResult = m_aEvaluators[i]->extractParamsQDF(hActionGroup);
                    qdf_closeGroup(hActionGroup);
                } else {
                    stdprintf("[MultiEvaluator<T>::extractParamsQDF] WARNING:  Couldn't open subgroup [%s]\n", m_aEvaluators[i]->getActionName());
                } 
            } else {
...
int MultiEvaluator<T>::writeParamsQDF(hid_t hSpeciesGroup) {
    ...
    for (int i = 0; i < m_iNumEvals && iResult == 0; i++) {
        ...
        if (iResult == 0) {
            ...
            if (this->m_pPop->getQDFVersionOut() >= 4) {
                hid_t hActionGroup = qdf_opencreateGroup(hSpeciesGroup, m_aEvaluators[i]->getActionName());
                if (hActionGroup != H5P_DEFAULT) {
                    iResult = m_aEvaluators[i]->writeParamsQDF(hActionGroup);
                    qdf_closeGroup(hActionGroup);
                } else {
                    stdprintf("[MultiEvaluator<T>::writeParamsQDF]Couldn't create subgroup [%s]\n", m_aEvaluators[i]->getActionName());
                    iResult = -1;
                }
            } else {
...
//
template<typename T>
int MultiEvaluator<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = 0;
    ...
    for (int i = 0; (i < m_iNumEvals) && (iResult == 0); i++) {
        iResult = getParamVal(mParams, m_vCombinationWeightNames[i], &m_adCombinationWeights[i]);
        if (iResult == 0) {
            modulemap::const_iterator it = mSubs.find(m_aEvaluators[i]->getActionName());
            if (it != mSubs.end()) {
                iResult = m_aEvaluators[i]->tryGetParams(it->second);
            } else { 
