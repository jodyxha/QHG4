...
//
template<typename T>
int PersHybLinearBirthRel<T>::operator()(int iAgentIndex, float fT) {
...
//
template<typename T>
int PersHybLinearBirthRel<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int PersHybLinearBirthRel<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int PersHybLinearBirthRel<T>::modifyParams(const std::string sAttrName, double dValue) {
    int iResult = 0;
...
//
template<typename T>
int PersHybLinearBirthRel<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_PERSHYBLINBIRTHREL_HYBMINPROB_NAME, &m_dHybMinProb);
    }
    return iResult;
}
