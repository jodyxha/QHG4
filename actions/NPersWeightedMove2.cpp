...
//
template<typename T>
NPersWeightedMove2<T>::NPersWeightedMove2(SPopulation<T> *pPop, SCellGrid *pCG, std::string sID, WELL512 **apWELL, LocEnv2<T> *pLE) 
    : Action<T>(pPop,pCG,ATTR_NPERSWEIGHTEDMOVE2_NAME, sID),
    m_apWELL(apWELL),
    m_pLE(pLE) {
...
//
template<typename T>
NPersWeightedMove2<T>::~NPersWeightedMove2() {
...
//
template<typename T>
int NPersWeightedMove2<T>::operator()(int iAgentIndex, float fT) {
    int iNewCellIndex = -1;
...
//
template<typename T>
int NPersWeightedMove2<T>::postLoop() {
    int iResult = 0;
    return iResult;
}
...
//
template<typename T>
bool NPersWeightedMove2<T>::isEqual(Action<T> *pAction, bool bStrict) {
    bool bEqual = true;
