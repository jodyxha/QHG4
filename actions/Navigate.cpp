    ...
    if ((pa->m_iLifeState > 0) && ((pa->m_iLifeState & LIFE_STATE_MOVING) == 0)) {
        ...
        if (it != m_mJumpProbs.end()) {
            ...
            if (i > 0) {
                ...
                if ((this->m_pCG->m_pGeography == NULL) || (!this->m_pCG->m_pGeography->m_abIce[iNewCellIndex])) {
                    ...
                    if (iNewCellIndex < 0) {
                        printf("[Navigate<T>::operator()] Attention: agent %d has cellindex %d\n", iAgentIndex, iNewCellIndex);
                    }
        ...
        for (itb = m_vCurBridges.begin(); itb != m_vCurBridges.end(); ++itb) {
            ...
            if (itb->first == iCellIndex) {
                ...
                //                printf("[Navigate<T>::operator()] have candidate at first\n");
            } else if (itb->second == iCellIndex) {
                iNewCellIndex = itb->first;
                //                printf("[Navigate<T>::operator()] have candidate at second\n");
            } else {
                iNewCellIndex = -1;
            }
            ...
            if (iNewCellIndex >= 0) {
                ...
                if (dR < m_dBridgeProb) {
                    this->m_pPop->registerMove(iCellIndex, iAgentIndex, iNewCellIndex);
                    //printf("[Navigate<T>::operator()] Sent %d over bridge from %d to %d at T%f\n", iAgentIndex, iCellIndex, iNewCellIndex, fT);
                }
            }
        }
    }
...
//
template<typename T>
int Navigate<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams,  ATTR_NAVIGATE_DECAY_NAME,   &m_dDecay);  
        iResult += getParamVal(mParams,  ATTR_NAVIGATE_DIST0_NAME,   &m_dDist0);  
        iResult += getParamVal(mParams,  ATTR_NAVIGATE_PROB0_NAME,   &m_dProb0);  
        iResult += getParamVal(mParams,  ATTR_NAVIGATE_MINDENS_NAME, &m_dMinDens);

        int iRes2 = getParamVal(mParams, ATTR_NAVIGATE_BRIDGE_PROB_NAME, &m_dBridgeProb);
        if (iRes2 != 0) {
            //no problem; use 0
        }
