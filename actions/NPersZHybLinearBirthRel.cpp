...
//
template<typename T>
int NPersZHybLinearBirthRel<T>::operator()(int iAgentIndex, float fT) {
...
//
template<typename T>
int NPersZHybLinearBirthRel<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int NPersZHybLinearBirthRel<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int NPersZHybLinearBirthRel<T>::modifyParams(const std::string sAttrName, double dValue) {
    int iResult = 0;
...
//
template<typename T>
int NPersZHybLinearBirthRel<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_NPERSZHYBLINBIRTHREL_HYBMINPROB_NAME, &m_dHybMinProb);
    }
    return iResult;
}
