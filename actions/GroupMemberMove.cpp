...
//
template<typename T>
int GroupMemberMove<T>::operator()(int iAgentIndex, float fT) {
    int iNewCellIndex = -1;
...
//
template<typename T>
int GroupMemberMove<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int GroupMemberMove<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int GroupMemberMove<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;
    

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = getParamVal(mParams, ATTR_GROUPMEMBERMOVE_PROB_NAME, &m_dMoveProb);
    }
    return iResult;
}
