...
//
template<typename T>
int BirthDeathRelComp<T>::operator()(int iAgentIndex, float fT) {
...
//
template<typename T>
int BirthDeathRelComp<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int BirthDeathRelComp<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int BirthDeathRelComp<T>::modifyParams(const std::string sAttrName, double dValue) {
...
//
template<typename T>
int BirthDeathRelComp<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_BIRTHDEATHRELCOMP_B0_NAME, &m_dB0);
        iResult += getParamVal(mParams, ATTR_BIRTHDEATHRELCOMP_D0_NAME, &m_dD0);
        iResult += getParamVal(mParams, ATTR_BIRTHDEATHRELCOMP_THETA_NAME, &m_dTheta);
        iResult += getParamStr(mParams, ATTR_BIRTHDEATHRELCOMP_THIS_NAME, m_sThis);
        iResult += getParamStr(mParams, ATTR_BIRTHDEATHRELCOMP_OTHER_NAME, m_sOther);
