...
//
template<typename T, class U>
int Genetics<T,U>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;
    std::string sTemp; 
    ...
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_GENETICS_GENOME_SIZE,       &m_iGenomeSize);
        iResult += getParamVal(mParams, ATTR_GENETICS_NUM_CROSSOVER,     &m_iNumCrossOvers);
        iResult += getParamVal(mParams, ATTR_GENETICS_MUTATION_RATE,     &m_dMutationRate);
        iResult += getParamVal(mParams, ATTR_GENETICS_CREATE_NEW_GENOME, &m_bCreateNewGenome);
        iResult += getParamVal(mParams, ATTR_GENETICS_BITS_PER_NUC,      &m_iBitsPerNuc);
    
        int iResult2 = getParamStr(mParams, ATTR_GENETICS_INITIAL_MUTS,  sTemp);
        if (iResult2 == 0) {
