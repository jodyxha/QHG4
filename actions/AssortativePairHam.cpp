...
//
template<typename T, class U>
int AssortativePairHam<T,U>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_ASSPAIRHAM_CUTOFF,         &m_fCutOff);
        int iPermute = 0;
        iResult += getParamVal(mParams, ATTR_ASSPAIRHAM_PERMUTE,        &iPermute);
        if (iPermute != 0) {
            m_bPermute = true;
        }
   }
