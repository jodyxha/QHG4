...
//
template<typename T>
int PersHybBirthDeathRel<T>::operator()(int iAgentIndex, float fT) {
...
//
template<typename T>
int PersHybBirthDeathRel<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int PersHybBirthDeathRel<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int PersHybBirthDeathRel<T>::modifyParams(const std::string sAttrName, double dValue) {
...
//
template<typename T>
int PersHybBirthDeathRel<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_PERSHYBBIRTHDEATHREL_HYBMINPROB_NAME, &m_dHybMinProb);
        iResult += getParamVal(mParams, ATTR_PERSHYBBIRTHDEATHREL_GROUPASS_NAME,   &m_dGroupAssProb);
        if (iResult == 0) {
            // just in case we're reading in new parameters,
