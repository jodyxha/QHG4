...
//
template<typename T>
int LinearBirth<T>::modifyParams(const std::string sAttrName, double dValue) {
    int iResult = 0;
...
//
template<typename T>
int LinearBirth<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {  
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_LINBIRTH_B0_NAME, &m_dB0);         
        iResult += getParamVal(mParams, ATTR_LINBIRTH_TURNOVER_NAME, &m_dTheta);
        iResult += getParamVal(mParams, ATTR_LINBIRTH_CAPACITY_NAME, &m_dK);    
    }
    return iResult;
}
