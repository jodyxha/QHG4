...
//
template<typename T>
int ConfinedMove<T>::modifyParams(const std::string sAttrName, double dValue) {
...
//
template<typename T>
int ConfinedMove<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_CONFINEDMOVE_X_NAME, &m_dX); 
        iResult += getParamVal(mParams, ATTR_CONFINEDMOVE_Y_NAME, &m_dY); 
        iResult += getParamVal(mParams, ATTR_CONFINEDMOVE_R_NAME, &m_dR); 
    }
    return iResult;
}
