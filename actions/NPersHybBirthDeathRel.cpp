...
//
template<typename T>
int NPersHybBirthDeathRel<T>::operator()(int iAgentIndex, float fT) {
...
//
template<typename T>
int NPersHybBirthDeathRel<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int NPersHybBirthDeathRel<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int NPersHybBirthDeathRel<T>::modifyParams(const std::string sAttrName, double dValue) {
...
//
template<typename T>
int NPersHybBirthDeathRel<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_NPERSHYBBIRTHDEATHREL_HYBMINPROB_NAME, &m_dHybMinProb);
        if (iResult == 0) {
            // just in case we're reading in new parameters,
