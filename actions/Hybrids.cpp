...
//
template<typename T, class U>
int Hybrids<T,U>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T, class U>
int Hybrids<T,U>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T, class U>
int Hybrids<T,U>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;
    ...
    if (this->checkParams(mParams) == 0) {
        ...
        iResult += getParamVal(mParams, ATTR_HYBRIDS_GENOME_SIZE,       &m_iGenomeSize);
        iResult += getParamVal(mParams, ATTR_HYBRIDS_NUM_CROSSOVER,     &m_iNumCrossOvers);
        iResult += getParamVal(mParams, ATTR_HYBRIDS_MUTATION_RATE,     &m_dMutationRate);
        iResult += getParamVal(mParams, ATTR_HYBRIDS_CREATE_NEW_GENOME, &m_bCreateNewGenome);
        iResult += getParamVal(mParams, ATTR_HYBRIDS_BITS_PER_NUC,      &m_iBitsPerNuc);
    
        int iResult2 = getParamStr(mParams, ATTR_HYBRIDS_INITIAL_MUTS,  sTemp);
        if (iResult2 == 0) {
        ...
        iResult2 = getParamStr(mParams, ATTR_HYBRIDS_INITIAL_ORIS,  sTemp);
        if (iResult2 == 0) {
