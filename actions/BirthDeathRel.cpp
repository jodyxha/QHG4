...
//
template<typename T>
int BirthDeathRel<T>::operator()(int iAgentIndex, float fT) {
...
//
template<typename T>
int BirthDeathRel<T>::extractParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int BirthDeathRel<T>::writeParamsQDF(hid_t hSpeciesGroup) {
...
//
template<typename T>
int BirthDeathRel<T>::modifyParams(const std::string sAttrName, double dValue) {
...
//
template<typename T>
int BirthDeathRel<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;

    const stringmap &mParams = pMC->getParams();
    if (this->checkParams(mParams) == 0) {
        iResult = 0;
        iResult += getParamVal(mParams, ATTR_BIRTHDEATHREL_B0_NAME, &m_dB0);
        iResult += getParamVal(mParams, ATTR_BIRTHDEATHREL_D0_NAME, &m_dD0);
        iResult += getParamVal(mParams, ATTR_BIRTHDEATHREL_THETA_NAME, &m_dTheta);
    }
