...
int ShareEvaluator<T>::extractParamsQDF(hid_t hSpeciesGroup) {
    ...
    if (!m_sPolyName.empty()) {
        stdprintf("ShareEvaluator::extractParamsQDF will work on %s\n", m_sPolyName);
        ...
        stdprintf("ShareEvaluator::extractParamsQDF: empty poly name\n");
        iResult = -1;
    }
    return iResult;
}
...
//
template<typename T>
int ShareEvaluator<T>::tryGetParams(const ModuleComplex *pMC) {
    int iResult = -1;
    ...
    if (this->checkParams(mParams) == 0) {
        ...
        iResult += getParamStr(mParams,  sKeyArrName, m_sArrayName);  
        iResult += getParamStr(mParams,  sKeyPolyName, m_sPolyName);  
